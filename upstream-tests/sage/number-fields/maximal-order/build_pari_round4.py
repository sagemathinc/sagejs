...
The trial bound controls only presentation of factors of the exact index.
Passing it as `[T, bound]` to `nfbasis` or `nfdisc` would instead ask
PARI for an order maximal only at those primes and must not be used here.
"""
...
jsonfac(F,trialbound)=my(s="[");for(i=1,matsize(F)[1],if(i>1,s=concat(s,","));s=concat(s,concat("[\"",concat(Str(F[i,1]),concat("\",",concat(Str(F[i,2]),concat(",\"",concat(if(F[i,1]<trialbound,"proven-prime",if(ispseudoprime(F[i,1]),"probable-prime","composite-unresolved")),"\"]"))))))));concat(s,"]");
...
emitcase(i,T,trialbound)=my(B,n,M,D,H,pd,fd,idx);B=nfbasis(T,&fd);n=poldegree(T);M=matrix(n,n,r,c,polcoef(B[r],c-1));D=denominator(M);H=mathnf((D*M)~)~;pd=poldisc(T);idx=sqrtint(abs(pd/fd));print("{\"ordinal\":",i,",\"coefficients\":",jsonvec(Vecrev(T)),",\"equation_discriminant\":\"",pd,"\",\"field_discriminant\":\"",fd,"\",\"index\":\"",idx,"\",\"local_index_factors\":",jsonfac(factor(idx,trialbound),trialbound),",\"basis_denominator\":\"",D,"\",\"basis_numerator\":",jsonmat(H),"}");
"""
