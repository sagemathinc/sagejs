...
try {
  ...
  assert.equal(await runSource([
    "import sagejs.runtime as runtime",
    "from sagejs.ffi.flint import fmpz_matrix, fmpz_matrix_nrows, fmpz_matrix_ncols, fmpz_matrix_set_entry, fmpz_matrix_entry, fmpz_matrix_copy, fmpz_matrix_mul, fmpz_matrix_det, fmpz_matrix_format, fmpz_matrix_serialize, fmpz_matrix_deserialize",
    "resource = fmpz_matrix(2, 2)",
    "fmpz_matrix_set_entry(resource, 0, 0, 2^100 + 17)",
    "fmpz_matrix_set_entry(resource, 0, 1, -3)",
    "fmpz_matrix_set_entry(resource, 1, 0, 5)",
    "fmpz_matrix_set_entry(resource, 1, 1, 7)",
    "resource_copy = fmpz_matrix_copy(resource)",
    "resource_square = fmpz_matrix_mul(resource, resource_copy)",
    "resource_format = fmpz_matrix_format(resource)",
    "resource_serialized = fmpz_matrix_serialize(resource_square)",
    "resource_restored = fmpz_matrix_deserialize(resource_serialized)",
    "print(fmpz_matrix_nrows(resource), fmpz_matrix_ncols(resource), fmpz_matrix_entry(resource, 0, 0) == 2^100 + 17)",
    "print(fmpz_matrix_entry(resource_square, 0, 0) == (2^100 + 17)^2 - 15, fmpz_matrix_det(resource) == 7*(2^100 + 17) + 15)",
    "print(fmpz_matrix_entry(resource_restored, 1, 1) == fmpz_matrix_entry(resource_square, 1, 1))",
    "resource.close(); resource_copy.close(); resource_square.close(); resource_format.close(); resource_serialized.close(); resource_restored.close()",
    `print(${liveCount})`,
    "def dense_zz_resource_slice():",
    "    value = matrix(ZZ, 2, [2, -3, 5, 7])",
    "    copy = value.__copy__()",
    "    value[0, 1] = -11",
    "    square = value * copy",
    "    restored = loads(dumps(square))",
    "    return (value._has_fmpz_matrix_resource(), restored._has_fmpz_matrix_resource(), value[0, 1], copy[0, 1], square.str(), value.det(), value.str(), restored.str() == square.str())",
    "print(dense_zz_resource_slice())",
  ].join("\n")), [
    "2 2 True",
    "True True",
    "True",
    "0",
    "(True, True, -11, -3, '[-51 -83]\\n[ 45  34]', 69, '[  2 -11]\\n[  5   7]', True)",
    "None",
  ].join("\n"));

  assert.equal(await runSource([
    "def dense_zz_failures_are_atomic():",
    "    value = matrix(ZZ, 2, [1, 2, 3, 4])",
    "    before = value.str()",
    "    entry_atomic = False",
    "    multiply_atomic = False",
    "    try:",
    "        value[9, 0] = 17",
    "    except IndexError:",
    "        entry_atomic = value.str() == before",
    "    try:",
    "        value * matrix(ZZ, 1, 1, [1])",
    "    except ValueError:",
    "        multiply_atomic = value.str() == before",
    "    return entry_atomic, multiply_atomic",
    "print(dense_zz_failures_are_atomic())",
  ].join("\n")), "(True, True)\nNone");
