...
test("loads the generated resource backend through the public package", () => {
  ...
  assert.ok(
    // Main's integer log/sqrt prefix resource adds 1,555 generated bytes;
    // the combined reviewed adapter is 430,645 bytes (formerly 429,090).
    resourceBackend.byteLength <= 432_000,
    `generated resource backend grew to ${resourceBackend.byteLength} bytes`,
  );
  assert.ok(
    // Canonical modular-abelian Hom/End codecs raise the reviewed minified
    // closure from 59,034 to 60,837 bytes. Keep a bounded ceiling; this is
    // serialization code, not an increase to the eager Wasm binary.
