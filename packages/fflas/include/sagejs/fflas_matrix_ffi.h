...
struct sagejs_fflas_float_allocation
{
    float *data;

    explicit sagejs_fflas_float_allocation(float *value) : data(value) {}

    ~sagejs_fflas_float_allocation()
    {
        if (data != NULL)
            FFLAS::fflas_delete(data);
    }

    sagejs_fflas_float_allocation(const sagejs_fflas_float_allocation &) = delete;
    sagejs_fflas_float_allocation &operator=(
        const sagejs_fflas_float_allocation &) = delete;
};
