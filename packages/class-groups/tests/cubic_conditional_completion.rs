...
fn prepare(coefficients: [i64; 4]) -> sagejs_class_groups::PreparedPublicCubic {
    prepare_monic_cubic(
...
fn candidate(
    prepared: &sagejs_class_groups::PreparedPublicCubic,
) -> sagejs_class_groups::AuthenticatedCubicPresentationCandidate {
...
fn sealed_result_answers_and_replays_arbitrary_ideal_queries() {
    ...
    assert!(matches!(
        ...
        sagejs_class_groups::PresentationZeroState::NonzeroInCurrentPresentation { .. }
    ));

    let mut counterfeit = certificate.clone();
    counterfeit.reduction.element[0] += 1;
    assert_eq!(
        completed.replay_ideal_class_certificate(&input, &counterfeit, limits, &mut workspace,),
