...
const basisNumerators = vector010Fixture.basis_numerator.flat();
const rowDenominators = Array(32).fill(vector010Fixture.basis_denominator);
...
basis_numerators = kernel_integer_buffer(
    packed_round4_power_basis_covolume,
    ${pythonIntegers(basisNumerators)},
)
row_denominators = kernel_integer_buffer(
    packed_round4_power_basis_covolume,
    ${pythonIntegers(rowDenominators)},
)
...
_covolume_warmup_ms = measure_covolume()
covolume_samples = [measure_covolume() for _ in range(7)]
covolume_samples.sort()
print(repr({
    ...
    'covolume_compiled': is_compiled(packed_round4_power_basis_covolume),
    'covolume_median_ms': covolume_samples[len(covolume_samples)//2],
    'covolume_status': integer_buffer_values(cov_control),
    'covolume_determinant': integer_buffer_values(determinant_parts),
}))
`;
