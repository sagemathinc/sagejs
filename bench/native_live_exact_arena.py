"""Neutral shared-budget exact-arena witnesses for Native Kernel v25."""
...
def live_arena_relation_step(
    ...
    with NativeExactArena(memory_limit) as workspace:
        relations = workspace.integer_matrix(2, 3)
        pivots = workspace.integer_vector(2)
        relations[0, 1] = left
        pivots[1] = right
...
def live_arena_shared_limit(memory_limit: uint64, value: int) -> int:
    ...
    with NativeExactArena(memory_limit) as workspace:
        matrix = workspace.integer_matrix(2, 2)
        vector = workspace.integer_vector(2)
        matrix[0, 0] = value
        vector[0] = value
