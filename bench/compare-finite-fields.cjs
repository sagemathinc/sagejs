...
const source = join(__dirname, "finite-fields.sage");
...
console.log(
  "operation runtime".padEnd(32),
  "median".padStart(11),
  "ns/operation".padStart(15),
...
for (const [label, timings] of results) {
  for (const [operation, entry] of timings) {
    ...
    console.log(
      `${operation.padEnd(18)} ${label.padEnd(12)}`.padEnd(32),
      `${(seconds * 1000).toFixed(2)} ms`.padStart(11),
      nanoseconds.toFixed(1).padStart(15),
      `${(nanoseconds / sageMedians.get(operation)).toFixed(2)}x`.padStart(11),
    );
  }
}
