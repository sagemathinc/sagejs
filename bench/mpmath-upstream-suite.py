...
module_selection_text = os.environ.get("SAGEJS_MPMATH_SUITE_MODULES", "")
if module_selection_text:
    selected_modules = set(module_selection_text.split(","))
    MODULES = tuple(module for module in MODULES if module in selected_modules)
elif selection is not None:
    selected_modules = set(name.split(".", 1)[0] for name in selection)
    MODULES = tuple(module for module in MODULES if module in selected_modules)
...
EXCLUDED_TESTS = set() if full_suite else {"test_convert.test_compatibility"}
...
for module_name in MODULES:
    ...
    try:
        ...
        print(
            "IMPORT\t" + module_name + "\tFAIL\t" + str(import_seconds) + "\t" + detail
        )
        print("MODULE\t" + module_name + "\t0\t1\t" + str(import_seconds) + "\t0.0")
        continue
    import_seconds = perf_counter() - import_started
    names = sorted(
        name
        ...
        and (selection is None or module_name + "." + name in selection)
    )
    passed = 0
    failed = 0
