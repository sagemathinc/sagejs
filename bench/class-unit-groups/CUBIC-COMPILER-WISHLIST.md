...
### 1. Resident small exact relation matrices — implemented
...
The integrated capability is not a faster scalar determinant call. It keeps
the source matrix resident through HNF support, exact transform replay, and the
bounded row-deletion schedule, then returns retained support plus rank in one
coarse boundary. A dynamic ordinary-Python oracle remains available. Native
and generated JavaScript match the canonical HNF lattice, FLINT source support,
and retained rows; CPython independently replays the retained lattice.

The nine-sample native evidence met the original target: the hard 4027
unconditional workflow improved by about 9.8%, with neighboring changes within
the 3% guard. See `RESIDENT-HNF.md` and
`test/number-field-class-group-resident-hnf.cjs` for the boundary, caps,
differential oracles, and exact receipts. The remaining items below are ranked
after that integrated improvement.
