...
The integrated cubic selector uses this independently callable small-matrix
capability. `resident_exact_relation_hnf_selection()` accepts
...
## Integrated caller and registries

`cubic_class_number._select_cubic_relation_candidates()` keeps its empty-input
guard and exception policy, calls the resident boundary once, and projects the
retained indices back to candidate records:
...
The production integration also:

1. Classifies `class_group_hnf.py` with the other matrix kernels and strict
   Python modules.
2. Exposes `bench:number-field-resident-hnf` for the comparison above. The
   benchmark retains the old selector locally as the `legacy` control and
   measures the integrated `auto` route separately.
3. Registers this production witness in `architecture/native-kernels.json`:
...
pack descriptor is needed. The capability manifest records the source kernel
under `kernel-flint`. No handwritten native file or
`architecture/native-code.json` exception is needed. This capability does not
consume the optimizer packed-container API; its resident `IntegerBuffer`
boundary is already owned by the source-transparent native compiler.
