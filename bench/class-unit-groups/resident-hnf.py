...
maximal order before the timer. The `legacy` route locally restores the
pre-integration selector; `auto` measures the current public route, and the
explicit resident routes select one backend exactly as described in
`RESIDENT-HNF.md`.
"""
...
TARGETS = tuple(
    ...
    for target in os.environ.get(
        "SAGEJS_RESIDENT_HNF_TARGETS", "legacy,auto,native,javascript"
    ).split(",")
    if target.strip()
)
...
if not TARGETS or any(
    target not in ("legacy", "auto", "native", "javascript", "python")
...
def legacy_selector(matrix_module, initial_rows, candidates, width):
    """Retain the pre-resident selector as an explicit benchmark control."""
    if not candidates:
        return (), 0
    ...
    try:
        ...
        while cursor < len(selected_indices):
            ...
            if target_index is not None and len(trial_rows) == width:
                ...
                same_lattice = (
                    matrix_module.exact_relation_hnf_basis(trial_rows, width) == basis
                )
            if same_lattice:
                selected_indices = trial_indices
            else:
                cursor += 1
        return tuple(candidates[index] for index in selected_indices), rank
...
def measured_legacy_selector(matrix_module, initial_rows, candidates, width):
    ...
    try:
        answer = legacy_selector(matrix_module, initial_rows, candidates, width)
    finally:
    ...
    for label, _coefficients, _expected in CASES:
        legacy = next(
            (
                ...
                and record["target"] == "legacy"
            ),
            None,
        )
        if legacy is None:
            continue
        for target in ("auto", "native", "javascript", "python"):
            resident = next(
                (
                    record
                    for record in records
            ...
            if resident is None:
                continue
            improvement = 1.0 - float(resident["seconds"]) / float(legacy["seconds"])
            comparisons.append(
                {
                    "label": label,
                    "proof": proof,
