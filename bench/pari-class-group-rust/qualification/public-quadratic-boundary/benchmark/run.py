...
def main() -> int:
    ...
    receipt = {
        ...
        "method": {
            ...
            "nativeThreading": "Rust cyclic and C2 x C(h/2) map construction for h>=10000 uses up to eight OS workers, capped by available parallelism. Native reduced-form enumeration and scalar class-number counting use up to eight workers when the candidate range has at least 20000 entries. Smaller Rust cases and Wasm use one worker. PARI has no matched worker pool, so ratios compare wall time, not equal CPU work.",
