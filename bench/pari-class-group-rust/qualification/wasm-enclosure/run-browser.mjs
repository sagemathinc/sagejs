...
const sampleCount = Number(process.env.SAGEJS_WASM_ENCLOSURE_SAMPLES ?? 15);
assert.ok(Number.isSafeInteger(sampleCount) && sampleCount >= 15 && sampleCount <= 100);

function statistics(values) {
  const sorted = [...values].sort((left, right) => left - right);
  const median = sorted[Math.floor(sorted.length / 2)];
  const deviations = sorted.map((value) => Math.abs(value - median)).sort((left, right) => left - right);
  return { samples: values, median, min: sorted[0], max: sorted.at(-1), mad: deviations[Math.floor(deviations.length / 2)] };
}
...
const server = http.createServer((request, response) => {
  ...
  if (pathname === "/") {
    ...
    response.end("<!doctype html><title>Sage.js Rust MPFR/MPC enclosure probe</title>");
    return;
  }
  const filename = routes.get(pathname);
  if (!filename) return response.writeHead(404).end("not found");
  response.writeHead(200, { "Content-Type": pathname.endsWith(".wasm") ? "application/wasm" : "text/javascript; charset=utf-8" });
    ...
    try {
      ...
      const result = await page.evaluate(async (samples) => {
        ...
        for (let sample = 0; sample < samples; sample += 1) {
          ...
          instance = await WebAssembly.instantiate(module, { wasi_snapshot_preview1: importObject });
          ...
          wasi.dispose();
          results.push({ status, stderr, payload, instantiateMs, computeMs, memoryPagesBefore, memoryPagesAfter });
        }
        return { compileMs, imports, userAgent: navigator.userAgent, results };
      }, sampleCount);
      const hashes = result.results.map((sample) => {
        assert.equal(sample.status, 0, sample.stderr);
        JSON.parse(sample.payload);
        return crypto.createHash("sha256").update(sample.payload).digest("hex");
      });
...
process.stdout.write(`${JSON.stringify({
  schema: "sagejs.rust-wasm-enclosure-browser-receipt/v1",
