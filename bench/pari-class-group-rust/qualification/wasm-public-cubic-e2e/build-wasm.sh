...
export RUSTFLAGS="-C target-feature=+simd128 -C link-arg=--export-memory -C link-arg=--initial-memory=16777216 -C link-arg=--max-memory=536870912"

cargo build --locked --release --target wasm32-wasip1 --lib
mkdir -p build
cp target/wasm32-wasip1/release/sagejs_rust_wasm_public_cubic_e2e.wasm \
  build/public-cubic-e2e.wasm
sha256sum build/public-cubic-e2e.wasm
