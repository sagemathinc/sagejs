...
| R2 relation/HNF loop | In progress | Exact relation witnesses are independently replayed; fixed-width Smith retries from an untouched checkpoint with GMP, and arbitrary-precision HNF/SNF returns exact transformation witnesses. General maximal-order small-norm plus deterministic PARI-style random continuation completes row 6's 1,130-dimensional relation rank with 1,526 rows, without answer-derived relations. Replacing the exact Rust 3D LLL oracle with pinned FLINT LLL reduces collection from about 110 s to about 71 s. A modularly independent square basis plus determinant-certified modular HNF reduces row-6 HNF from 252.55 s to 35.61 s including determinant computation and incorporation of all 396 remaining relations; the complete candidate route is now 109.42 s and preserves exact Smith factors `[2, 2]` and class number 4. Transform evidence and faithful subfactor growth/restart policy remain open. |
| R3 completion and maps | In progress | Verified Smith transforms produce exact maps on small supplied presentations. Row 6 now has a compact candidate map assigning all 1,130 factor-base generators coordinates in `C2 x C2`; an independent Rust pass verifies that all 1,526 original relations map to zero. HNF provenance transforms, principal elements, order witnesses, units, regulator/completion proof, saturation, generator ideals, and proof that the presentation is the full class group remain unimplemented. |
