...
| W0 row-6 relation prefix | Passed as a bounded development stage; memory risk | A 691,342-byte Wasm artifact advances the same answer-free row-6 input through production numerical preparation, enumeration, exact norm/valuation work, and relation-cache insertion. Fifteen calls in native Rust and each browser agree on 203 structural rows plus one collected relation, with 926 rank directions explicitly missing. Medians are 285.7 ms native, 690.5 ms Chromium, 4,709 ms Firefox, and 625 ms WebKit. Browser linear memory reaches 3,357 pages (about 220 MB), close to the 256 MiB product budget even for this prefix; reducing workspace high-water and Firefox's 16.48x ratio are blocking browser tasks. |
