...
| R3 completion and maps | In progress | Verified Smith transforms produce exact maps on small supplied presentations. Row 6 now has a compact candidate map assigning all 1,130 factor-base generators coordinates in `C2 x C2`; an independent Rust pass verifies that all 1,137 accepted relations map to zero. It also has independently replayed order-two relation combinations for concrete factor-base generators of both invariant coordinates, with compact principal elements represented as products of the collected algebraic generators to signed powers. Units, regulator/completion proof, honesty/saturation, usable ideal maps, final generator-ideal construction, and proof that the presentation is the full class group remain unimplemented. |
