...
| R2 relation/HNF loop | In progress | Exact relation witnesses are independently replayed; fixed-width Smith retries from an untouched checkpoint with GMP, and arbitrary-precision HNF/SNF returns exact transformation witnesses. General maximal-order small-norm plus deterministic PARI-style random continuation completes row 6's 1,130-dimensional relation rank with 1,526 rows in 109.22 s on the captured Smith run, without answer-derived relations. A qualification-only bridge to the pinned FLINT build reduces that exact presentation to Smith factors `[2, 2]` and class number 4, matching PARI, but the direct dense reduction takes 257.96 s and does not yet return transforms. Scalable sparse HNF, transform evidence, and faithful subfactor growth/restart policy remain open. |
