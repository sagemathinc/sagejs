...
| R2 relation/HNF loop | In progress | Exact relation witnesses are independently replayed; fixed-width Smith retries from an untouched checkpoint with GMP, and arbitrary-precision HNF/SNF returns exact transformation witnesses. General maximal-order small-norm plus deterministic PARI-style random continuation now completes row 6's 1,130-dimensional modular relation rank with 1,526 rows in 111.94 s, without answer-derived relations. Scalable sparse HNF and faithful subfactor growth/restart policy remain open. |
