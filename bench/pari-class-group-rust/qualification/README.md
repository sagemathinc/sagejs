...
| R2 relation/HNF loop | In progress | Exact relation witnesses are independently replayed; fixed-width Smith retries from an untouched checkpoint with GMP, and arbitrary-precision HNF/SNF returns exact transformation witnesses. General maximal-order small-norm plus deterministic continuation completes row 6's 1,130-dimensional relation rank with the intended seven surplus rows, without answer-derived relations. Feeding the modular rank cache's four unresolved pivot coordinates back into random ideal selection reduces collection from 71.15 s and 1,526 rows to 7.64 s and 1,137 rows. A modularly independent square basis plus determinant-certified modular HNF yields the exact `[2, 2]` Smith map; the complete candidate route is now 48.56 s. A staged provenance pass obtains exact order-relation coefficients without the rejected multi-gigabyte dense transform; fusing its duplicated initial HNF remains open. |
| R3 completion and maps | In progress | Verified Smith transforms produce exact maps on small supplied presentations. Row 6 now has a compact candidate map assigning all 1,130 factor-base generators coordinates in `C2 x C2`; an independent Rust pass verifies that all 1,137 accepted relations map to zero. It has replayed order-two principal witnesses for both invariant coordinates. The saturated relation kernel yields seven exact compact unit candidates; stable high-precision rational reconstruction and flattened exact replay recover a rank-two unit lattice. A direct 4,096-bit Arb pass rigorously encloses its regulator from the exact compact products, agreeing with PARI 2.17.4. The reconstruction denominator bound remains heuristic, and Belabas--Friedman analytic completion, honesty, certified saturation, usable ideal maps, and final generator-ideal construction remain open. |
