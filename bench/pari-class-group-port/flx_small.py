...
@native
def pari_flx_copy(w: IntegerBuffer, a: int, da: int, out: int) -> int:
    if out > a:
...
@native
def pari_flx_normalize(w: IntegerBuffer, a: int, da: int, p: int, out: int) -> int:
    if da < 0:
        raise ValueError("cannot normalize zero polynomial")
    if w[a + da] == 1:
        return pari_flx_copy(w, a, da, out)
    inv = pari_word_mod_inverse(w[a + da], p)
    for i in range(da):
        w[out + i] = w[a + i] * inv % p
    w[out + da] = 1
    for i in range(da + 1, 9):
...
@native
def pari_flx_sub(
    w: IntegerBuffer, a: int, da: int, b: int, db: int, p: int, out: int
) -> int:
    d = da
    if db > d:
        d = db
...
@native
def pari_flx_sqr(w: IntegerBuffer, a: int, da: int, p: int, out: int) -> int:
    if da > 4:
...
@native
def pari_flx_divrem(
    w: IntegerBuffer, a: int, da: int, b: int, db: int, p: int, quot: int, rem: int
) -> int:
    return _pari_flx_divrem(w, a, da, b, db, p, quot, rem, 1)


@native
def pari_flx_div(
    w: IntegerBuffer, a: int, da: int, b: int, db: int, p: int, out: int, scratch: int
) -> int:
    """Source quotient-only branch; scratch is one disjoint nine-slot owner."""
    _pari_flx_divrem(w, a, da, b, db, p, out, scratch, 0)
    d = da - db
    if d < -1:
        d = -1
    while d >= 0 and w[out + d] == 0:
...
def pari_flx_rem(
    ...
    if db == 0:
        return pari_flx_copy(w, a, -1, out)
    return _pari_flx_divrem(w, a, da, b, db, p, scratch, out, 1)


@native
def _pari_flx_divrem(
    w: IntegerBuffer,
    a: int,
    da: int,
    b: int,
    ...
    if da < db and db != 0:
        if want_rem == 0:
            return -1
        return pari_flx_copy(w, a, da, rem)
    inv = 1
    if w[b + db] != 1:
        inv = pari_word_mod_inverse(w[b + db], p)
    if db == 0:
        for i in range(da + 1):
...
def pari_flx_gcd(
    ...
    if db > da:
        a, b = b, a
        da, db = db, da
    pari_flx_copy(w, a, da, scratch)
    pari_flx_copy(w, b, db, scratch + 9)
    while db >= 0:
        if db == 0:
            return pari_flx_copy(w, scratch + 9, 0, out)
        dc = pari_flx_divrem(w, scratch, da, scratch + 9, db, p, scratch + 18, out)
        pari_flx_copy(w, scratch + 9, db, scratch)
        pari_flx_copy(w, out, dc, scratch + 9)
        da = db
        db = dc
    return pari_flx_copy(w, scratch, da, out)


@native
def pari_flx_deriv(w: IntegerBuffer, a: int, da: int, p: int, out: int) -> int:
    for i in range(9):
        w[out + i] = 0
...
@native
def pari_flx_deflate(w: IntegerBuffer, a: int, da: int, k: int, out: int) -> int:
    if k < 1:
