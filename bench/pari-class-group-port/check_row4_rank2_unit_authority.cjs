...
const api = require("./row4_rank2_unit_authority_coordinator.cjs");

const ROOT = path.resolve(__dirname, "../..");
const COORDINATOR = path.join(__dirname, "row4_rank2_unit_authority_coordinator.cjs");
if (process.argv.length !== 4) {
  process.stderr.write("usage: check_row4_rank2_unit_authority.cjs PRESENTATION ROW4_W0\n");
  process.exit(2);
}
...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "row4-rank2-unit-"));

function clone(value) { return JSON.parse(JSON.stringify(value)); }
function reject(mutate) {
  const changed = clone(owner); mutate(changed);
  assert.throws(() => api.verifyOwner(changed, presentation, owner.ancestry));
}
...
try {
  ...
  reject(value => { value.units.unitKernelTransform[7] = String(BigInt(value.units.unitKernelTransform[7]) + 1n); });
  ...
  reject(value => { value.units.unitRealSigns[0] *= -1; });
  ...
  // Instrument the producer's event selector.  A second fully capped replay
  // proves that it asks for prepared/factor-base data only, never the frozen
  // fundamental-unit or terminal-result events.
...
m._event=event
o=m.compose_row4_rank2_unit_authority(p,m.PRESENTATION_SHA256,w,m.W0_SHA256)
