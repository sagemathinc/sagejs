...
(async()=>{
 const cases=[];
 for(const degree of [3,4])for(const distinguished of [0,1,2,3])for(const power of [0,1,2,3])for(const status of [0,-3,1,2,-1,-2]){
  ...
  const order=[2,1,3].filter(j=>!(j===distinguished&&(power+1)%(j===2?1:degree)===0&&(j===2?1:degree)===degree));
  const wanted=status===0||status===-3?order:order.slice(0,1);
  cases.push({degree,distinguished,power,status,wanted});
 }
...
for c in json.load(sys.stdin):
 ...
 while True:
  j=f([3,1,2],[c['degree'],1,c['degree']],[1,1,1],c['degree'],c['distinguished'],c['power'],s,cur,cnt,p)
  if not j: break
 ...
 before=(s[:],cur[:],cnt[:],p[:]);assert f([3,1,2],[c['degree'],1,c['degree']],[1,1,1],c['degree'],c['distinguished'],c['power'],s,cur,cnt,p)==0
...
`,path.resolve(__dirname,'../..'),path.resolve(__dirname,'../../src/lib')],{input:JSON.stringify(cases),encoding:'utf8',timeout:30000});assert.equal(python.status,0,python.stderr);
 ...
 for(const c of cases)for(const backend of ['javascript','gmp']){
  ...
  const args=[[3n,1n,2n],[BigInt(c.degree),1n,BigInt(c.degree)],[1n,1n,1n],BigInt(c.degree),BigInt(c.distinguished),BigInt(c.power),s,cur,cnt,p];
