"""PARI 2.17.4 RgM_inv real-domain basecase, square dimensions 1..5.
...
@native
def pari_row23_regulator_inverse(
    ...
    """
    if n < 1 or n > 5:
        raise ValueError("inverse prototype dimension outside 1..5")
    if (
        len(work) < 3 * n * n
        or len(rhs) < 3 * n * n
        or len(output) < 3 * n * n
