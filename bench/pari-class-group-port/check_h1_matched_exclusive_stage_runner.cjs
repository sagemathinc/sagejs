...
function arm(implementation, offset = 0n) {
  ...
  const terminalShape = implementation === "sagejs" ? runner.sageTerminalShape(authority) : runner.pariTerminalShape(authority);
  const raw = rawTrace(implementation, offset); const partitions = runner.durationPartitions(implementation, raw);
  return runner.validateArm({ implementation, rootDurationNanoseconds: String(raw.reduce((sum, item) => sum + item.nanoseconds, 0n)), durationPartitions: partitions, partitionTotalsNanoseconds: Object.fromEntries(raw.map(item => [runner.stageLabel(implementation, item.stage), String(item.nanoseconds)])), terminalShapeSha256: runner.digest(terminalShape), sourceAuthoritySha256: runner.digest(authority), terminalStatus: "pari-correspondence-complete-internal-h1" });
}
function receipt() {
  const sage = sageOwners(); const pari = pariRecord(); const terminalShape = runner.sageTerminalShape(sage);
  ...
  return { schema: "sagejs.pari-class-group/h1-matched-duration-partitions-development-v2", diagnosticOnly: true, qualifiedTiming: false, boundaryQualification: "unqualified-development-host", fieldId: runner.FIELD_ID, seedPolicy: { requestedSeed: "1", sagejsActualRngStateAvailable: false, pariActualRngStateAvailable: true }, inputProvenance: { sourcePath: "/tmp/input.json", fileSha256: runner.FROZEN_INPUT_FILE_SHA256, preparedInputSha256: runner.FROZEN_PREPARED_INPUT_SHA256 }, buildProvenance, authorities: { sagejs: { replayOwners: sage, replayOwnersSha256: runner.digest(sage), terminalShape, terminalShapeSha256: runner.digest(terminalShape), rngAuthority: { available: false }, workAuthority: { sha256: runner.digest(sage) } }, pari: { pristineRecord: pari, pristineRecordSha256: runner.digest(pari), terminalShape: runner.pariTerminalShape(pari), terminalShapeSha256: runner.digest(runner.pariTerminalShape(pari)), rngAuthority: pari.rng, workAuthority: pari.work }, commonTerminalShapeSha256: runner.digest(terminalShape) }, warmup: { policy: "one excluded active diagnostic call per persistent prepared implementation; Sage uses preparation replay", arms: [arm("sagejs"), arm("pari")] }, pairs, summary: runner.deriveSummary(pairs) };
}
...
function main() {
  const adapter = fs.readFileSync(path.join(__dirname, "h1_unified_complete_adapter.cjs"), "utf8"); assert.match(adapter, /diagnosticStageTrace\(\)/); assert.match(adapter, /replayDiagnosticStageTrace/); assert.match(adapter, /sourceAuthority/);
  ...
  rejected("independent terminal shape", value => { value.authorities.pari.terminalShape.classGroup.classNumber = "2"; }, /deep-equal/);
