...
const SOURCE = path.join(__dirname, "row10_prepared_initial_root.py");
const DEGREE = 4;
...
function parseSignature(source) {
  const match = source.match(/def pari_row10_prepared_initial_root\(([\s\S]*?)\n\) -> int:/);
...
async function computePreparedInitialRoot(payload, options = {}) {
  ...
  const fn = module.pari_row10_prepared_initial_root;
  ...
  const owners = {
    ...
    embedding_m: buffer(DEGREE ** 2, 16, prepared.admission_matrix_m.map(BigInt)),
    embedding_p: buffer(DEGREE ** 2, 4, prepared.admission_matrix_p.map(BigInt)),
    embedding_e: buffer(DEGREE ** 2, 16, prepared.admission_matrix_e.map(BigInt)),
    ...
    index_workspace: buffer(12000, 32),
    index_descriptors: buffer(DEGREE * (3 + DEGREE + DEGREE ** 2), 32),
    index_ranks: buffer(DEGREE, 4), index_state: buffer(4, 4),
  ...
  const owner = {
    schema: "sagejs.pari-class-group/row10-prepared-initial-owner-v1",
