"""Row-20 unreduced ideal packets through PARI 2.17.4 small_norm collection.
...
row-20 prebuilt-packet corridor.
"""
...
@native
def pari_collect_row20_unreduced_ideals(
    ...
    """Visit the exact row-20 original packets from a fresh relation cache.
    ...
    if (
        ...
        or outer_mode != 0
        or precision != 192
        or admission_real_count != 1
        or admission_mode != 2
        or len(relation) != 7
        or len(relation_state) < 6
        or relation_state[0] != 3
        or relation_state[5] != 14
        or packets != 7
        or search_count != 7
        or len(search_ideals) < 7
        or len(packet_ideals) != 7 * 25
        or len(packet_norms) != 7
    ):
        raise ValueError("unsupported row-20 unreduced ideal corridor")
    for i in range(7):
        if packet_ids[i] < 1 or packet_ids[i] > 7:
            raise ValueError("invalid row-20 packet identity")
        if search_ideals[i] < 1 or search_ideals[i] > 7:
            raise ValueError("invalid row-20 search identity")
        for j in range(i):
            if packet_ids[j] == packet_ids[i]:
                raise ValueError("duplicate row-20 packet identity")
            if search_ideals[j] == search_ideals[i]:
                raise ValueError("duplicate row-20 search identity")
