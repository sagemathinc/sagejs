...
const fieldAt=process.argv.indexOf('--field');
assert(fieldAt<0 || ['0','1'].includes(process.argv[fieldAt+1]),'--field must be 0 or 1');
const selectedField=fieldAt<0?-1:Number(process.argv[fieldAt+1]);
...
int main(int argc,char **argv){char *tail;long reps=1,selected=-1;
if(argc>3)return 2;
if(argc>1){reps=strtol(argv[1],&tail,10);if(!argv[1][0]||*tail)return 2;}
if(argc>2){selected=strtol(argv[2],&tail,10);if(!argv[2][0]||*tail)return 2;}
if(reps<1||reps>10000||selected < -1||selected>1)return 2;
pari_init(256000000,10000);DEBUGLEVEL=0;
const char *polynomials[]={"x^3-20018*x+20034","x^3-20010*x+20018"};
for(long field=0;field<2;field++){if(selected>=0&&field!=selected)continue;pari_sp outer=avma;
...
/* One untimed fresh-state warmup, then exactly reps timed computations. */
for(long rep=-1;rep<reps;rep++){pari_sp keep=avma;FB_t G=F;G.perm=leafcopy(F.perm);G.L_jid=search;
RELCACHE_t cache={0};FACT *fact=(FACT*)stack_malloc((F.KC+1)*sizeof(FACT));fact[0].pr=0;
audit_small=audit_fact=audit_ideals=0;struct timespec begin,end;if(rep>=0)clock_gettime(CLOCK_MONOTONIC,&begin);
...
GEN cyc=ZM_snf(W);if(rep>=0){clock_gettime(CLOCK_MONOTONIC,&end);total+=elapsed(begin,end);}
...
assert(path.isAbsolute(compilerPath),'compiler must resolve to an absolute path');
const buildArguments=[...flags,'-I'+path.join(pari,'src/headers'),'-I'+lib,c,'-L'+lib,'-Wl,-rpath,'+lib,'-lpari','-lm','-o',exe];
const build={compilerPath,compilerVersion:run(compilerPath,['--version']).trim(),flags,
  arguments:buildArguments,includePaths:[path.join(pari,'src/headers'),lib],
  libraryDirectory:lib,pariLibraryPath:fs.realpathSync(path.join(lib,'libpari.so')),
  libraries:['pari','m'],sourcePath:c,executablePath:exe};
run(compilerPath,buildArguments);
const executableArguments=[String(repetitions),String(selectedField)];
const trace=run(exe,executableArguments);fs.writeFileSync(path.join(dir,'stdout.jsonl'),trace);const outputs=trace.trim().split('\n').map(JSON.parse);
assert.deepEqual(outputs.map(r=>r.field),selectedField<0?[0,1]:[selectedField]);
...
Object.assign(summary,{build,executableArguments,selectedField:selectedField<0?'both':selectedField,
  warmupCallsPerField:1,
  rerunUsage:'oracle REPETITIONS FIELD (FIELD = -1 for both, 0 or 1); one untimed fresh-state warmup per field; outputs remain unqualified'});
