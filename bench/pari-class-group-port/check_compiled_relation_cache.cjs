...
long before=C.last-C.base,gen=step%3!=0,rnd=step%2,k=add_rel_i(&C,R,nz,gen?gen_1:NULL,0,0,NULL,rnd);
...
for r in json.load(sys.stdin):
    ...
    got=f(R,nz,gen,0,0,rnd,state,basis,records,hashes,metadata,scratch)
...
const got=mod.pari_prepared_add_relation[backend](R.map(BigInt),BigInt(nz),BigInt(gen),0n,0n,BigInt(rnd),state,basis,records,hashes,metadata,scratch);
...
console.log('192 resident add_rel_i transitions match PARI/CPython/JS/GMP, including signed vectors and duplicate/zero/dependent paths');
