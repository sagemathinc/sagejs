...
function sourceOracle(pari, archive, item) {
  ...
  ]) assert.equal(sha(run("tar", ["-xOf", archive, name])), wanted);
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "sagejs-unit-bridge-"));
  const library = path.join(pari, "Olinux-x86_64");
  const source = String.raw`
#include "pari.h"
#include "paripriv.h"
static GEN re(GEN x){return typ(x)==t_COMPLEX?gel(x,1):x;}
static GEN im(GEN x){return typ(x)==t_COMPLEX?gel(x,2):gen_0;}
static void scalar(GEN x){long e=0;x=re(x);if(typ(x)==t_INT)pari_printf("[\"%Ps\",-1,0]",x);else if(!signe(x))printf("[\"0\",0,%ld]",expo(x));else pari_printf("[\"%Ps\",%ld,%ld]",mantissa_real(x,&e),bit_prec(x),expo(x));}
static long phase(GEN x){GEN y=im(x);if(gequal0(y))return 0;double q=gtodouble(y)/M_PI;long n=lround(q);if(fabs(q-n)>1e-7)pari_err_BUG("unit bridge phase");n%=2;if(n<0)n+=2;return n;}
...
static void ivec(GEN x){putchar('[');for(long i=1;i<lg(x);i++){if(i>1)putchar(',');pari_printf("\"%Ps\"",gel(x,i));}putchar(']');}
static void imat(GEN x){putchar('[');for(long j=1;j<lg(x);j++)for(long i=1;i<lgcols(x);i++){if(j>1||i>1)putchar(',');pari_printf("\"%Ps\"",gcoeff(x,i,j));}putchar(']');}
int main(){pari_init(256000000,10000);long prec=nbits2prec(192),retry=nbits2prec(2048);GEN b0=bnfinit0(gp_read_str("${item.polynomial}"),1,NULL,prec),b=bnfnewprec(b0,retry),nf=bnf_get_nf(b),A=bnf_get_logfu(b),fu=bnf_get_fu(b);printf("{\"logs\":");smat(A);printf(",\"phases\":");pmat(A);printf(",\"units\":[");for(long j=1;j<lg(fu);j++){if(j>1)putchar(',');ivec(algtobasis(nf,gel(fu,j)));}printf("],\"regulator\":");scalar(bnf_get_reg(b));printf(",\"embedding\":");smat(nf_get_M(nf));printf(",\"tensor\":[");for(long j=1;j<=3;j++){if(j>1)putchar(',');imat(zk_multable(nf,col_ei(3,j)));}puts("]}");pari_close();}
`;
...
(async()=>{
  ...
  assert.equal(Math.max(...oracle.expected.logs.map((x)=>Number(x[1]))),2176);
  ...
  for(const backend of ["javascript","gmp","tagged"]){
    ...
    assert.equal(bridge.pari_cubic_unit_compose_provenance[backend](prep.u,BigInt(item.columns),factor.factor,out),0n);
    const snapshot={u:values(prep.u),clean:values(prep.clean),phases:prep.phases.slice(),factor:values(factor.factor),provenance:values(out),getfuState:initial.state.slice()};
...
o=I(2*c);assert b.pari_cubic_unit_compose_provenance(a[6],c,f[1],o)==0
print(json.dumps({'units':2,'relations':q['resident_state']['relation_count'],'phaseBits':sum(a[11])}))
`,root,path.join(root,"src/lib")],{input:JSON.stringify({item,expected:oracle.expected})}));
  console.log(JSON.stringify({cases:1,backends:["cpython","javascript","gmp","tagged"],dynamic,traceSha256:oracle.traceSha256,
    exactUnits:"pristine-source oracle",exactNorms:true,exactLogs:"resident bridge input",composedProvenance:true,coreBytes:fs.statSync(bridgeBuilt.coreSourcePath).size,
    residentGetfuStatus:"PRECI",remaining:{precisionRetry:"requires packed p2176 logs / p2240 embedding and p2304 working capacity",mixedSignature:"not connected",wideSelector:"not exercised"},artifactDirectory:oracle.directory}));
