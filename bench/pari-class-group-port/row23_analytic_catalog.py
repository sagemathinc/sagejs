"""Row-23 maximal-order prime-degree catalog for analytic normalization.

Copyright (C) The PARI group. GPL-2.0-or-later, without warranty.

This bounded degree-five bridge derives every local residue-degree pattern
from the authenticated maximal-order multiplication table at the equation
index prime.  Away from that prime it uses the Kummer polynomial path, so no
frozen analytic pattern is an input.
"""
...
@native
def _row23_poly_degree(work: IntegerBuffer, offset: int, bound: int) -> int:
...
def _row23_poly_remainder(
    ...
    return _row23_poly_degree(work, output, divisor_degree - 1)


@native
def _row23_poly_gcd_degree(
    work: IntegerBuffer,
    left: int,
    left_degree: int,
    right: int,
    ...
    while b_degree >= 0:
        r_degree = _row23_poly_remainder(
            work,
            first,
            a_degree,
            second,
...
@native
def _row23_squarefree_polynomial(
    ...
    derivative_degree = _row23_poly_degree(work, derivative, 4)
    if derivative_degree < 0:
        ...
        return 1
    gcd_degree = _row23_poly_gcd_degree(
        work,
        source,
        5,
        derivative,
...
def _row23_x_power_mod(
    ...
    while exponent != 0:
        if exponent % 2 != 0:
            result_degree = _row23_poly_mulmod(
                work,
                result,
                result_degree,
                base,
        ...
        exponent //= 2
        if exponent != 0:
            base_degree = _row23_poly_mulmod(
                work,
                base,
                base_degree,
                base,
...
@native
def pari_row23_analytic_degree_catalog(
    polynomial: IntegerBuffer,
    table: IntegerBuffer,
    ...
    """Publish grouped residue-degree counts for the row-23 quintic.
    ...
    if prime_count < 1 or len(state) < 4:
        raise ValueError("invalid row23 analytic catalog dimensions")
    if (
        len(polynomial) < 6
        or len(table) < 125
        ...
        raise ValueError("short row23 analytic catalog storage")
    previous = 1
    for i in range(prime_count):
        if primes[i] <= previous or primes[i] > 3037000493:
            raise ValueError("invalid row23 analytic prime catalog")
        previous = primes[i]
    state[0] = -1
    state[1] = 0
    state[2] = 0
    ...
    for at in range(prime_count):
        ...
        if prime == 131:
            ...
            difference = 3192
            local = 3000
            squarefree_degree = _row23_squarefree_polynomial(
            ...
            if squarefree_degree > 1:
                power_degree = _row23_x_power_mod(
                    workspace,
                    squarefree,
                    squarefree_degree,
                    prime * prime,
                ...
                difference_degree = _row23_poly_degree(
            ...
            if remainder_degree != 0:
                if remainder_degree < 3 or remainder_degree > 5:
                    ...
                    state[2] = linear
                    state[3] = gcd_degree
                    raise ValueError("invalid row23 residual factor degree")
...
__all__ = ["pari_row23_analytic_degree_catalog"]
