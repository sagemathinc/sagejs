...
const ROWS = 421, DEGREE = 4, PLACES = 3, FIRST_COLUMNS = 427;
...
const APPEND_SHAPES = Object.freeze([
  Object.freeze({ oldState: Object.freeze([2, 9, 418, 1, 7, 45, 0, 427, 0]),
    newColumns: 1 }),
  Object.freeze({ oldState: Object.freeze([3, 10, 418, 0, 7, 0, 0, 428, 0]),
    newColumns: 2 }),
]);
...
function validateBoundary(prepared, root) {
  ...
  assert.equal(root.schema, "sagejs.pari-class-group/row11-prepared-initial-owner-v1");
  assert.equal(root.authority.preparedAuthoritySha256, prepared.authoritySha256);
  assert.deepEqual(root.rootState, ["1", "3019", "3019", "421", "259", "259",
    "421", "4", "421", "24", "428", "404", "4", "397", "421", "0",
    "1048576", "65537"]);
  assert.deepEqual(root.relations.state, ["24", "4330", "397", "7", "0", "428"]);
  assert.equal(root.relations.denseRecords.length, 24 * ROWS);
  ...
  assert.deepEqual(root.factor.subfactor, ["4", "5", "2", "7"]);
  ...
  assert.equal(root.capacity.logicalRecordCapacity, 4330);
}
...
function collectorInput(preparedEnvelope, root) {
  ...
  return {
    ...
    outer_mode: 1, outer_ru: PLACES, log_precision: 192, scalar_prefix_count: 24,
    ...
    relation_state: ["24", "4330", "397", "7", "0", String(FIRST_COLUMNS)],
    relation_basis: root.relations.basis,
    relation_records: [...root.relations.denseRecords,
      ...Array((4330 - 24) * ROWS).fill("0")],
    relation_hashes: [...root.relations.hashes, ...Array(4330 - 24).fill("0")],
    relation_metadata: [...root.relations.metadata, ...Array((4330 - 24) * 3).fill("0")],
    generators: [...root.relations.generators, ...Array((4330 - 24) * DEGREE).fill("0")],
...
function prepareHnfaddTransaction(storage, append, state, newColumns, explicit) {
  ...
  assert(shape, "unreviewed row-11 hnfadd transaction shape");
...
async function runPreparedGateC(prepared, root, options = {}) {
  ...
  const checkpoints = [snapshot(427)], expected = [428, 430], passTrace = [];
  ...
  let checkpointIndex = 0, collectionPasses = 1, squash = 0, retryReset = false;
  while (checkpointIndex < expected.length) {
    collectionPasses += 1;
    assert(collectionPasses <= 4,
      `row-11 continuation exceeded reviewed pass bound: ${JSON.stringify(passTrace)}`);
    // Row 11 needs two one-ideal continuation probes to close the first
    // target, then PARI acceptance action 1 opens a three-ideal retry window.
    ...
    const { search, outerPerm, outer, cache, schedule, completed, control, perm } =
      controlStorage;
    control.fill(0n);
    if (checkpointIndex === 1 && !retryReset) {
      squash = 0;
      retryReset = true;
    }
    ...
    const desiredSearch = checkpointIndex === 0 ? 1 : 3;
    for (let index = 0; index < desiredSearch; index += 1) {
      writeIntegerSlot(search, index, resident.perm[index]);
      writeIntegerSlot(outerPerm, index, resident.perm[index]);
    }
    nextControl[0] = desiredSearch;
    squash += 1;
    if (checkpointIndex === 1)
      writeIntegerSlot(cv.relation_state, 5, resident.state[7] + 2);
    cv.search_count = BigInt(desiredSearch); cv.outer_mode = 1n;
    cv.outer_ru = BigInt(PLACES); cv.scalar_prefix_count = 24n;
    ...
    assert.equal(columns, expected[checkpointIndex], JSON.stringify(passTrace));
