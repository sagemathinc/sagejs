...
async function runFirstHnf(prepared, factorOwner, { hnfWords = 16 } = {}) {
  assert.equal(factorOwner.schema,
    "sagejs.pari-class-group/row21-prepared-factor-base-v1");
  ...
  const input = {
    ...
    log_precision: 192, initial_additional: 8, initial_target: TARGET,
    hnf_k0: 4,
  ...
  assert(ownerBytes < 512*1024**2, "row-21 first-HNF owners exceed 512 MiB");
  const view = value => value.toArray ? value.toArray() : Array.from(value);
  let status;
  try {
    status = fn.gmp(...names.map(([name]) => values[name]));
  } catch (error) {
    error.row21State = Object.fromEntries([
