...
for i,(r,w) in enumerate(zip(*json.load(sys.stdin))):
 a=int(r['a']);b=int(r['b']);f=m.pari_word_inverse_generator if r['kind'] else m.pari_word_bezout
...
`,path.resolve(__dirname,'../..'),path.resolve(__dirname,'../../src/lib')],{input:JSON.stringify([cases,expected])});
 ...
 for(const [i,r]of cases.entries())for(const backend of ['javascript','gmp']){
  const f=r.kind?m.pari_word_inverse_generator:m.pari_word_bezout;assert.equal(f.nativeAvailable,true);
