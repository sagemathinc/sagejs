...
from sagejs.native import IntegerBuffer, UInt64Buffer, checked_uint64, native

from .bounded_flx_small_factor import (
    bounded_pari_flx_small_degfact,
    bounded_pari_flx_small_factor_workspace_size,
)
...
def bounded_pari_get_fs_small(
    ...
    if equation_index % prime == 0:
        ...
        or len(word_workspace) < 9 + bounded_pari_flx_small_factor_workspace_size()
        or len(word_metadata) < 9 + bounded_pari_flx_small_factor_workspace_size()
    ...
    if prime == 2:
        ...
        number = pari_f2x_small_degfact(
            int(binary), factor_degrees, factor_exponents, exact_workspace, 9
        )
    else:
        word_prime = checked_uint64(prime)
        for i in range(degree + 1):
            word_workspace[i] = checked_uint64(coefficients[i] % prime)
        number = bounded_pari_flx_small_degfact(
            word_workspace,
            word_metadata,
            0,
            degree,
