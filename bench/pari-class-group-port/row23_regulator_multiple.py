...
from .row23_regulator_determinant import pari_row23_regulator_determinant
from .row23_regulator_inverse import pari_row23_regulator_inverse
...
@native
def pari_row23_regulator_multiple(
    logs: IntegerBuffer,
    rows: int,
    columns: int,
    degree: int,
    ...
    """
    if rows < 2 or rows > 5 or columns < 0 or degree < rows or degree > 2 * rows:
    ...
    for j in range(kept):
        if rank_pivots[j] != 0:
            ...
            target += 1
    status = pari_row23_regulator_determinant(
    ...
    if pari_regulator_exponent(em, ep, ee) - pari_regulator_exponent(dm, dp, de) > -20:
        ...
        state[0] = 3
        return 3
    status = pari_row23_regulator_inverse(
