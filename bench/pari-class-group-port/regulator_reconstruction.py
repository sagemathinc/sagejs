...
@native
def pari_regulator_denominator_lcm(x: int, y: int) -> int:
...
@native
def pari_regulator_denominator_power(value: int, rank: int) -> int:
    """Positive `powiu` restricted to ranks 1–3.
...
@native
def pari_regulator_bad_check(m: int, p: int, e: int) -> int:
...
@native
def pari_regulator_reconstruction(
    ...
    """
    if rows < 1 or rows > 3 or columns < 1:
    ...
    for j in range(columns):
        ...
        for i in range(rows):
            ...
            if i == 0:
                ...
                column_den = pari_regulator_denominator_lcm(column_den, d)
        if j == 0:
            den = column_den
        elif column_den != 1:
    ...
    reason = pari_regulator_bad_check(cm, cp, ce)
    state[1] = 5
    state[0] = reason
    if reason != 0:
        return reason
