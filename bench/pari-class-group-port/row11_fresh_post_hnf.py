"""Fresh prepared-only row-11 post-HNF acceptance boundary.
...
def _checked_checkpoint(checkpoint: dict) -> tuple[int, int, int, list[int], list[int]]:
    columns = int(checkpoint["columns"])
    if columns not in (428, 430):
        raise ValueError("row-11 checkpoint has an unexpected column count")
    state = [int(value) for value in checkpoint["state"]]
    if state[0] not in (2, 3) or state[2] not in (418, 419) or state[7] != columns:
        raise ValueError("row-11 checkpoint state changed")
    h = [int(value) for value in checkpoint["h"]]
    if len(h) != state[0] * state[0]:
        raise ValueError("row-11 retained HNF has the wrong shape")
    c = [int(value) for value in checkpoint["c"]]
    if len(c) != 7 * PLACES * columns:
        raise ValueError("row-11 transformed-log owner has the wrong shape")
    return columns, state[0], state[2], h, c
...
def accept_fresh_row11_checkpoints(
    checkpoints: list[dict], inverse_hr: list[int]
) -> list[dict]:
    """Run the same-invocation row-11 rejection/retry transaction."""

    if len(checkpoints) != 2:
        raise ValueError("row-11 acceptance requires exactly three checkpoints")
...
def fresh_row11_terminal(
    ...
    attempts = accept_fresh_row11_checkpoints(checkpoints, inverse_hr)
    if [entry["status"] for entry in attempts] != [5, 0]:
        raise ValueError("row-11 acceptance schedule changed")
    return {
