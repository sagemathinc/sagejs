"""Prepared row-13 factor base and rational-relation root.
...
# ceiling fixed independently of the row-13 answer KC=999.  Logical views are
# derived only after KC has been computed inside the call.
ROW13_PREPARED_MAX_IDEALS = 1024
ROW13_PREPARED_MAX_ADDITIONAL = 16


@native
def pari_row13_prepared_initial_root(
    ...
    """Compute the row-13 prepared prefix through its 54 rational relations.
    ...
    if len(root_state) < 18 or root_state[0] != 0:
        raise ValueError("row13 prepared root requires a fresh state owner")
    # This is the explicit fail-closed replacement for the still-untranslated
    # general automorphism/minidx corridor.
    if (
        len(polynomial) != 5
        or polynomial[0] != -20000000010
        or polynomial[1] != -20000000006
        or polynomial[2] != 0
        or polynomial[3] != 0
        or polynomial[4] != 1
        or discriminant != -4320000007232000005404800002002560000290992
        or real_places != 2
        or complex_pairs != 1
        or precision != 256
        or equation_index != 1
        ...
        raise ValueError("unsupported row13 prepared field corridor")
    ...
    if (
        ...
        raise ValueError("row13 root requires the neutral complete prime prefix")
    if len(base_state) < 7 or len(root_state) < 18:
        raise ValueError("short row13 prepared policy state")
    if (
        len(relation) < ROW13_PREPARED_MAX_IDEALS
        or len(relation_scratch) < ROW13_PREPARED_MAX_IDEALS
        or len(relation_basis) < ROW13_PREPARED_MAX_IDEALS * ROW13_PREPARED_MAX_IDEALS
    ):
        raise ValueError("short row13 neutral relation admission owners")
    maximum_records = (
        10 * (ROW13_PREPARED_MAX_IDEALS + ROW13_PREPARED_MAX_ADDITIONAL) + 50
    )
    if (
        len(relation_records) < maximum_records * ROW13_PREPARED_MAX_IDEALS
        ...
        raise ValueError("short row13 neutral relation record owners")
    ...
    if status != 0:
        raise ValueError("row13 degree catalog failed")
    ...
    if kcz != kcz2:
        raise ValueError("row13 unequal-bound honesty corridor is unsupported")
    if kc < 1 or kc > ROW13_PREPARED_MAX_IDEALS:
        raise ValueError("row13 factor base exceeds public admission ceiling")
    ...
    if written != kc:
        raise ValueError("row13 generated descriptor count mismatch")
    ...
    if (
        ...
        raise ValueError("short row13 active prime group owners")
    for i in range(kcz):
    ...
    if subcount < 1 or len(subfactor) < subcount or len(minidx) < kc:
        raise ValueError("short row13 subfactor/minidx owner")
    for i in range(subcount):
        subfactor[i] = permutation[i]
    for i in range(kc):
        # Authenticated row-13 specialization: no nontrivial automorphism.
        minidx[i] = i + 1

    root_state[0] = -6
    unit_rank = real_places + complex_pairs - 1
    additional = 5 + unit_rank
    if additional > ROW13_PREPARED_MAX_ADDITIONAL:
        raise ValueError("row13 relation surplus exceeds public admission ceiling")
    ...
    # PARI's independent BNF_RELPID scheduling default.  Row 14 happened to
    # have a subfactor count of four; the two quantities are not coupled.
    nrelid = 4
