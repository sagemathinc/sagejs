...
// Data-only C7 join for real-cubic development-panel row 4.  Input JSON does
// not authorize itself: every component is admitted through a detached replay
// capability, and publication uses the field-neutral correspondence contract.

const neutral = require("./class_unit_correspondence_result.cjs");

const COMPOSITION_SCHEMA =
  "sagejs.pari-class-group/row4-c7-result-composition-v1";
const INPUT_REPLAY_SCHEMA =
  "sagejs.pari-class-group/row4-c7-input-replay-v1";
const PUBLICATION_REPLAY_SCHEMA =
  "sagejs.pari-class-group/row4-c7-publication-replay-v1";
const PRESENTATION_SCHEMA =
  "sagejs.pari-class-group/row34-real-cubic-presentation-v1";
const CLASS_SCHEMA =
  "sagejs.pari-class-group/row4-real-cubic-class-witness-v1";
const UNIT_SCHEMA =
  "sagejs.pari-class-group/row4-rank2-unit-authority-v1";
const FIELD_ID =
  "generated-sha256-806defcf929c9cfff7467b8e7ea7b9f939cdd042bce8c1f5a310f5688904e3b9";
const POLYNOMIAL = Object.freeze(["20000000018", "-20000000010", "0", "1"]);
...
const CLASS_OWNER_SHA256 =
  "dd9f39ffd204329908a5acd0117e61d9431aa814c17b702a4c992172676cde3a";
const UNIT_OWNER_SHA256 =
  "81fd6b103d706ca9a62b215503c896f38f28698bda4ef0e0d16a557b9f632343";
const INTEGER = /^(0|-?[1-9][0-9]*)$/;
const SHA256 = /^[0-9a-f]{64}$/;

class Row4C7CompositionFailure extends Error {}
function fail(message) { throw new Row4C7CompositionFailure(message); }
...
function openBoundary(input, label, expectedSha256) {
  ...
  if (ownerSha256 !== expectedSha256 ||
      authority.replaySchema !== INPUT_REPLAY_SCHEMA ||
      typeof authority.replay !== "function") fail(`${label} authority changed`);
  let receipt;
  try { receipt = authority.replay(structuredClone(owner)); }
  catch (error) {
    throw new Row4C7CompositionFailure(`${label} replay rejected`, { cause: error });
  }
...
function validatePresentation(boundary) {
  ...
  if (value.schema !== PRESENTATION_SCHEMA || value.field?.id !== FIELD_ID ||
      value.field?.panelIndex !== 4) fail("row-4 presentation identity changed");
  equal(value.field.polynomial, POLYNOMIAL, "row-4 polynomial");
  equal(value.field.signature, [3, 0], "row-4 signature");
  equal(value.dimensions, { degree: 3, places: 3, factorBaseSize: 560,
    relationCount: 567, kernelRank: 7, unitRank: 2,
    classPresentationDimension: 1, subfactorCount: 4 }, "row-4 dimensions");
  if (value.presentation?.classNumber !== "2") fail("row-4 class number changed");
  equal(integers(value.presentation.invariants, "invariants", 1), ["2"], "invariants");
  equal(integers(value.presentation.terminalW, "presentation", 1), ["2"], "presentation");
...
function validateClass(boundary, presentation) {
  ...
  equal(value.quotient, { presentation: ["2"], generatorOrder: "2",
    properDivisorRejected: "1", generatorNontrivial: true }, "class quotient");
  if (value.generator?.terminalIndex !== 0 || value.generator?.sourceIndex !== 2 ||
      value.generator?.norm !== "5") fail("class generator changed");
  ...
  const factorIdeals = integers(presentation.owner.factorBase?.ideals,
    "presentation factor-base ideals", 560 * 9);
  // The presentation stores factor-base ideals in terminal order.  Its first
  // permutation entry is source id 3 (zero-based sourceIndex 2).
  if (presentation.owner.replay?.terminalPermutation?.[0] !== "3")
    fail("terminal/source class-generator map changed");
  equal(generatorIdeal, factorIdeals.slice(0, 9), "class generator join");
  const compact = plain(value.compactPrincipalWitness, "compact class witness");
  const relationIndices = integers(compact.relationIndices,
    "class relation indices", 397);
  const relationExponents = integers(compact.relationExponents,
    "class relation exponents", 397);
  const principalGenerators = integers(compact.principalGenerators,
    "class principal generators", 397 * 3);
  if (compact.kind !== "signed-retained-relation-product" ||
      compact.factorCount !== 397 || compact.expandedGeneratorMaterialized !== false ||
  ...
  return { ...opened, generatorIdeal, principalGenerators, relationExponents,
    relationIndices, target };
}
...
function validateUnit(boundary, presentation) {
  ...
  const provenance = integers(units.rawUnitProvenance, "factored unit provenance", 567 * 2);
...
function joinEvidence(inputs) {
  const source = plain(inputs, "row-4 C7 inputs");
...
function completePayload(evidence, replaySchema) {
  const p = evidence.presentation.owner;
  const c = evidence.klass;
  const u = evidence.unit;
  const storage = [
...
function prepareRow4C7Result(inputs, options = {}) {
  ...
  const componentOwnerSha256 = {
    classOwner: evidence.klass.ownerSha256,
    presentation: evidence.presentation.ownerSha256,
    unitOwner: evidence.unit.ownerSha256,
  };
  const mathematicalAuthoritySha256 = neutral.sha256Canonical({
    classOrder: "2", componentOwnerSha256,
    ...
    normalizedInvariants: ["2"], unitNorms: evidence.unit.norms,
  });
...
function publishPreparedRow4C7Result(prepared, authority, publisher = undefined) {
  const value = plain(prepared, "prepared row-4 C7 result");
...
module.exports = {
  CLASS_OWNER_SHA256, COMPOSITION_SCHEMA, FIELD_ID, INPUT_REPLAY_SCHEMA,
  PARI_SOURCE_SHA256, POLYNOMIAL, PRESENTATION_SHA256,
  PUBLICATION_REPLAY_SCHEMA, Row4C7CompositionFailure, UNIT_OWNER_SHA256,
  joinEvidence, prepareRow4C7Result, publishPreparedRow4C7Result,
};
