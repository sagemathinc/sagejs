...
async function runFirstHnf(prepared, factorOwner,
  ...
  const source = path.join(__dirname, "row23_connected_relation_hnf.py");
  ...
  assert.equal(descriptors.length, ROWS);
  assert.equal(factor.ideals.length, ROWS);
  assert.equal(factor.norms.length, ROWS);
  assert.equal(factor.permutation.length, ROWS);
  ...
  assert.equal(rationalPrimes.length, 20);
  ...
  const primeTableLength = Math.max(...rationalPrimes.map(Number)) + 1;
  const primeOffsets = Array(primeTableLength).fill("-1");
  const primeCounts = Array(primeTableLength).fill("0");
  ...
  const input = {
    ...
    log_precision: 192, initial_additional: 9, initial_target: TARGET,
    hnf_k0: 4,
  ...
  try {
    ...
    error.row23State = Object.fromEntries([
