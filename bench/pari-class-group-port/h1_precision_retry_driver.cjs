...
function sunitArguments(fn, exact, matrix, precision) {
  ...
  return {
    ...
    arguments: [
      ...
      I(fn, 512), I(fn, 512), I(fn, 512), I(fn, 512), I(fn, 128),
      S(fn, 4), state,
    ],
  };
}

function unitArguments(fn, exact, rebuiltLogs, matrix, precision) {
  const cleanLogs = [];
  ...
  return {
    ...
    state,
    arguments: [
      I(fn, 18, cleanLogs), I(fn, 27, embedding),
      I(fn, 27, exact.multiplicationBasis), precision,
      I(fn, 18), I(fn, 6), I(fn, 4), I(fn, 18), I(fn, 18), I(fn, 27),
      I(fn, 18), I(fn, 18), I(fn, 6), I(fn, 9), I(fn, 3), I(fn, 6),
      units, logs, state, S(fn, 3),
      F(fn, 4), F(fn, 4), F(fn, 2), F(fn, 6), I(fn, 2), F(fn, 4),
      I(fn, 2), I(fn, 3), I(fn, 3), F(fn, 3), F(fn, 3), I(fn, 4),
      I(fn, 4), I(fn, 4), I(fn, 2), I(fn, 512), I(fn, 512), I(fn, 512),
      I(fn, 512), I(fn, 512), I(fn, 128),
    ],
  };
}
...
async function createH1PrecisionRetryDriver(options = {}) {
  ...
  const builds = await Promise.all([
    ...
    compile({ sourcePath: path.join(HERE, "unit_reconstruction_cubic.py") }),
  ...
  const getfuFn = require(builds[2].modulePath).pari_getfu_real_cubic;
  ...
  return function runH1PrecisionRetry(input) {
    ...
    for (;;) {
      ...
      const rebuilt = sunitArguments(sunitFn, exact, matrix, precision);
      ...
      const unit = unitArguments(
        getfuFn, exact, values(rebuilt.output), matrix, precision,
      );
      ...
      if (next > resourceCap) next = resourceCap;
      if (next <= precision)
...
module.exports = { createH1PrecisionRetryDriver, exactDigest };
