...
SCHEMA = "sagejs.pari-class-group/row11-fresh-rank2-c5-c6-v1"
...
LIVE_TRACE_SCHEMA = "sagejs.pari-class-group/development-default-driver-trace-v1"
FIELD_ID = (
...
def _prepared(live_trace: Mapping[str, Any]) -> tuple[list[int], list[int], list[int]]:
    prepared = _event(live_trace["events"], "prepared")
...
def _c6(c5: Mapping[str, Any], live_trace: Mapping[str, Any]) -> dict[str, Any]:
    embedding_real, embedding_imag, tensor = _prepared(live_trace)
    work = {
        name: _zeros(length)
        for name, length in {
            "expReal": 18,
...
def compose_row11_rank2_c5_c6(
    ...
    lane_a_sha256: str,
    live_trace: Mapping[str, Any],
    live_trace_sha256: str,
) -> dict[str, Any]:
    lane_a = _mapping(lane_a, "Lane A owner")
    live_trace = _mapping(live_trace, "same-invocation live trace")
    if (
        lane_a.get("schema") != LANE_A_SCHEMA
        or lane_a.get("field", {}).get("id") != FIELD_ID
    ):
        raise Row11Rank2Failure("wrong Lane A owner")
    if live_trace.get("schema") != LIVE_TRACE_SCHEMA:
        raise Row11Rank2Failure("wrong same-invocation live trace")
    ...
    events = live_trace.get("events")
    if not isinstance(events, list):
        raise Row11Rank2Failure("same-invocation live events missing")
    ...
    return {
        ...
        "ancestry": {
            ...
            "sameInvocationTraceSha256": live_trace_sha256,
