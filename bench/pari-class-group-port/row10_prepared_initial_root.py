"""Prepared row-10 factor base and rational-relation root.
...
# Public admission policy for this experimental root.  The power-of-two
# ceiling is part of the input-independent corridor; logical views are derived
# only after KC has been computed inside the call.
ROW10_PREPARED_MAX_IDEALS = 1024
ROW10_PREPARED_MAX_ADDITIONAL = 16


@native
def pari_row10_prime_degree_catalog(
    ...
    """Build the degree catalog, including the maximal-order prime above 37."""
    stride = 3 + degree + degree * degree
    capacity = prime_count * degree
    if (
        degree != 4
        or equation_index != 37
        or index_prime != 37
        or index_count < 1
        or index_count > degree
        or len(index_descriptors) < index_count * stride
        ...
        raise ValueError("short row10 index-prime degree catalog")
    groups = 0
    factors = 0
    for i in range(prime_count):
        ...
        if equation_index % prime == 0:
            if prime != index_prime:
                raise ValueError("unexpected row10 equation-index divisor")
            previous = 0
            for j in range(index_count):
                residue_degree = index_descriptors[j * stride + 2]
                if residue_degree < 1 or residue_degree > degree:
                    raise ValueError("invalid row10 index-prime residue degree")
                if residue_degree < previous:
                    raise ValueError("unsorted row10 index-prime decomposition")
            ...
            if result != 0:
                raise ValueError("row10 ordinary degree catalog failed")
...
@native
def pari_row10_prepared_initial_root(
    ...
    """Compute the row-10 prepared prefix through its 26 rational relations.
    ...
    if len(root_state) < 18 or root_state[0] != 0:
        raise ValueError("row10 prepared root requires a fresh state owner")
    # This is the explicit fail-closed replacement for the still-untranslated
    # general automorphism/minidx corridor.
    row10 = (
        len(polynomial) == 5
        and polynomial[0] == -2000042
        and polynomial[1] == -2000022
        and polynomial[2] == 0
        and polynomial[3] == 0
        and polynomial[4] == 1
        and discriminant == -315574182938393724979760
    )
    if (
        not row10
        or real_places != 2
        ...
        raise ValueError("unsupported row10 prepared field corridor")
    ...
    if (
        ...
        raise ValueError("row10 root requires the neutral complete prime prefix")
    if len(base_state) < 7 or len(root_state) < 18:
        raise ValueError("short row10 prepared policy state")
    if (
        len(relation) < ROW10_PREPARED_MAX_IDEALS
        or len(relation_scratch) < ROW10_PREPARED_MAX_IDEALS
        or len(relation_basis) < ROW10_PREPARED_MAX_IDEALS * ROW10_PREPARED_MAX_IDEALS
    ):
        raise ValueError("short row10 neutral relation admission owners")
    maximum_records = (
        10 * (ROW10_PREPARED_MAX_IDEALS + ROW10_PREPARED_MAX_ADDITIONAL) + 50
    )
    if (
        len(relation_records) < maximum_records * ROW10_PREPARED_MAX_IDEALS
        ...
        raise ValueError("short row10 neutral relation record owners")

    root_state[0] = -1
    index_count = pari_prepared_index_prime_descriptors(
        basis_table,
        degree,
        37,
        real_places,
    ...
    if index_count != 3:
        raise ValueError("row10 index-prime component count changed")
    status = pari_row10_prime_degree_catalog(
        polynomial,
        degree,
        equation_index,
        runtime_primes,
    ...
    if status != 0:
        raise ValueError("row10 degree catalog failed")
    ...
    if kcz != kcz2:
        raise ValueError("row10 unequal-bound honesty corridor is unsupported")
    if kc < 1 or kc > ROW10_PREPARED_MAX_IDEALS:
        raise ValueError("row10 factor base exceeds public admission ceiling")

    root_state[0] = -3
    pari_random_seed(random_state, 1)
    index_prime_position = 0
    while (
        index_prime_position < prime_count
        and runtime_primes[index_prime_position] != 37
    ):
        index_prime_position += 1
    if index_prime_position >= prime_count:
        raise ValueError("row10 index prime missing from runtime prefix")
    index_pattern_position = pattern_offsets[index_prime_position]
    index_pattern_count = pattern_counts[index_prime_position]
    if index_pattern_count != 2 or full_counts[index_prime_position] != 3:
        raise ValueError("row10 index-prime catalog identity changed")
    saved_degree0 = pattern_degrees[index_pattern_position]
    saved_degree1 = pattern_degrees[index_pattern_position + 1]
    saved_multiplicity0 = pattern_multiplicities[index_pattern_position]
    saved_pattern_count = pattern_counts[index_prime_position]
    pattern_counts[index_prime_position] = 1
    pattern_degrees[index_pattern_position] = degree
    pattern_multiplicities[index_pattern_position] = index_count
    ...
    pattern_degrees[index_pattern_position] = saved_degree0
    pattern_degrees[index_pattern_position + 1] = saved_degree1
    pattern_multiplicities[index_pattern_position] = saved_multiplicity0
    pattern_counts[index_prime_position] = saved_pattern_count
    stride = 3 + degree + degree * degree
    index_slot = full_offsets[index_prime_position]
    for j in range(index_count):
        source = j * stride
        slot = index_slot + j
        catalog_primes[slot] = index_descriptors[source]
        catalog_e[slot] = index_descriptors[source + 1]
        catalog_f[slot] = index_descriptors[source + 2]
        catalog_inert[slot] = 0
        for i in range(degree):
            catalog_generators[slot * degree + i] = index_descriptors[source + 3 + i]
        for a in range(degree):
            for b in range(degree):
                catalog_tau[slot * degree * degree + a * degree + b] = (
                    index_descriptors[source + 3 + degree + b * degree + a]
                )
    requested_counts[index_prime_position] = index_count
    written += index_count
    kummer_state[3] = written
    if written != kc:
        raise ValueError("row10 generated descriptor count mismatch")
    ...
    if (
        ...
        raise ValueError("short row10 active prime group owners")
    for i in range(kcz):
    ...
    if subcount < 1 or len(subfactor) < subcount or len(minidx) < kc:
        raise ValueError("short row10 subfactor/minidx owner")
    for i in range(subcount):
        subfactor[i] = permutation[i]
    for i in range(kc):
        # Authenticated row-14 specialization: no nontrivial automorphism.
        minidx[i] = i + 1

    root_state[0] = -6
    unit_rank = real_places + complex_pairs - 1
    additional = 5 + unit_rank
    if additional > ROW10_PREPARED_MAX_ADDITIONAL:
        raise ValueError("row10 relation surplus exceeds public admission ceiling")
