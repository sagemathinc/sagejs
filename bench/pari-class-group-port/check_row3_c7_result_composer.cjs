...
const composer = require("./row3_c7_result_composer.cjs");
const neutral = require("./class_unit_correspondence_result.cjs");
const presentationApi = require("./row34_real_cubic_presentation_coordinator.cjs");
const classApi = require("./row3_real_cubic_class_witness_coordinator.cjs");
const unitApi = require("./row3_rank2_unit_authority_coordinator.cjs");
...
function immutablePublish(raw, directory) {
  ...
  const destination = path.join(directory, `row3-c7-class-unit-result-${digest}.json`);
...
function worker(argv) {
  const [presentationPath, classPath, unitPath,
    outputDirectory = path.join(os.tmpdir(), "sagejs-row3-c7-owner")] = argv;
  assert(presentationPath && classPath && unitPath,
    "usage: check_row3_c7_result_composer.cjs PRESENTATION CLASS UNIT [OUTPUT]");
  const data = {
  ...
  const prepared = composer.prepareRow3C7Result(inputs);
  ...
  assert.deepEqual(stores.get("class-presentation").entries, ["3", "0", "0", "2"]);
  assert.equal(stores.get("class-order-relation-indices").entries.length, 443);
  assert(stores.get("class-order-relation-exponents").entries.some(value => BigInt(value) < 0n));
  assert.equal(stores.get("factored-unit-provenance").entries.length, 1350);
  assert.deepEqual(stores.get("unit-norms").entries, ["1", "-1"]);
  assert.deepEqual(stores.get("regulator-enclosure").entries, data.unitOwner.regulator.packed);
  ...
  function finalReplay(candidate) {
    const replay = composer.prepareRow3C7Result(inputs);
  ...
  const published = composer.publishPreparedRow3C7Result(prepared, authority(raw), publisher);
  assert.equal(composer.publishPreparedRow3C7Result(prepared, authority(raw), publisher), published);
  ...
  function authorityMutation(name, mutate) {
    ...
    assert.throws(() => composer.prepareRow3C7Result(boundaries(data, { [name]: changed })),
      composer.Row3C7CompositionFailure); mutationsRejected += 1;
  }
  authorityMutation("presentation", value => { value.presentation.terminalW[0] = "4"; });
  authorityMutation("classOwner", value => { value.quotient.generatorOrder = "3"; });
  ...
  function semanticMutation(name, mutate) {
    const changed = structuredClone(data[name]); mutate(changed);
    assert.throws(() => composer.prepareRow3C7Result(
  ...
  function rejectReseal(mutate) {
    ...
    assert.throws(() => composer.publishPreparedRow3C7Result(
  ...
  const source = fs.readFileSync(path.join(__dirname, "row3_c7_result_composer.cjs"), "utf8");
  ...
  process.stdout.write(`${JSON.stringify({
    schema: "sagejs.pari-class-group/row3-c7-result-check-v1",
    classNumber: 6, invariantFactors: [6], generatorOrder: 6, unitRank: 2,
