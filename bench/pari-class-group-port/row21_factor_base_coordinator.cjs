...
// Authenticated prepared-field -> row-21 factor-base transaction.
...
const SCHEMA = "sagejs.pari-class-group/row21-prepared-factor-base-v1";
...
function publish(directory, owner) {
  ...
  const destination = path.join(directory, `row21-prepared-factor-base-${ownerSha256}.json.gz`);
...
async function run(payload) {
  if (!payload || JSON.stringify(Object.keys(payload).sort()) !==
    ...
    throw new Error("unreviewed row21 factor-base payload");
  ...
  if (authority.sha256 !== PREPARED_SHA256 ||
    ...
    throw new Error("row21 prepared authority changed");
  const p = payload.prepared;
  if (p.n !== "5" || p.prep_index !== "846" || p.admission_real_count !== "3")
    throw new Error("row21 prepared identity changed");
  ...
  for (const fn of [factor, descriptorsFn, initial, hnf, subfactor])
    if (!fn.nativeAvailable) throw new Error("row21 native function unavailable");
  ...
  if (primeCount < 0) throw new Error("row21 neutral prime catalog has no 257 sentinel");
  primeCount += 1;
  ...
  for (const prime of primes) {
    ...
    if (846 % prime === 0) {
      ...
      const count = Number(descriptorsFn.gmp(table, BigInt(prime), 3n, matrixM, matrixP,
        matrixE, 3n, descriptorWorkspace, packed, ranks, state));
  ...
  if (state[7] !== 1n || kcz !== kcz2 || kc !== kc2 || c2 >= primes.at(-1))
    throw new Error("row21 initial-base publication failed");
  ...
  for (const prime of selectedRational) {
    ...
    if (!value) {
      ...
      const count = Number(descriptorsFn.gmp(table, BigInt(prime), 3n, matrixM, matrixP,
        matrixE, BigInt(limit), descriptorWorkspace, packed, ranks, descriptorState));
      value = { count, packed: packedSlice(packed, 0, count * 33) };
      decomposition.set(prime, value);
    }
    const needed = Number(packedAt(counts, prime));
    if (needed < 1 || needed > value.count) throw new Error("row21 descriptor prefix mismatch");
    for (let j = 0; j < needed; j += 1) {
      const record = value.packed.slice(j * 33, (j + 1) * 33);
      if (Number(record[0]) !== prime) throw new Error("row21 descriptor prime mismatch");
      selected.push(record);
    }
  }
  if (selected.length !== kc) throw new Error("row21 selected descriptor count mismatch");
  const ideals = [], norms = [];
  ...
  const owner = {
    ...
    field: { polynomial: p.prep_polynomial, discriminant: p.analytic_discriminant,
      degree: "5", signature: ["3", "1"], precision: p.precision },
