...
async function compiled(sourceName, exportName) {
  ...
  const built = await compileKernel({ sourcePath: source,
    cacheRoot: "/scratch/sagejs-native-cache-row23-acceptance" });
...
async function analyticInverseHr(prepared) {
  const catalog = await compiled("row23_analytic_catalog.py",
    "pari_row23_analytic_degree_catalog");
...
async function runAcceptance(prepared, liveResult) {
  ...
  assert.deepEqual(liveResult.relationState, ["40", "450", "0", "1", "0", "40"]);
  assert.deepEqual(liveResult.chainState, [3, 0, 0, 40]);
  assert.deepEqual(liveResult.hnfState, [1, 10, 30, 0, 9, 3, 0, 40, 0]);
  ...
  const kernel = await compiled("row23_post_hnf_acceptance.py",
    "pari_row23_post_hnf_acceptance");
  const fn = kernel.fn;
  ...
  const explicit = {
    ...
    h: liveResult.values.hnf_result_h,
