...
from .bounded_flx_small import (
    ...
    bounded_pari_flx_mul,
    bounded_pari_flx_sqr,
    bounded_pari_flx_rem,
)
...
def bounded_pari_flxq_mul(
    ...
    d = bounded_pari_flx_mul(w, a, da, b, db, p, scratch)
    return bounded_pari_flx_rem(w, scratch, d, t, dt, p, out, scratch + 9)


@native
def bounded_pari_flxq_sqr(
    w: UInt64Buffer, a: int, da: int, t: int, dt: int, p: uint64, out: int, scratch: int
) -> int:
    """Source square then remainder; reserve 18 disjoint scratch slots."""
    d = bounded_pari_flx_sqr(w, a, da, p, scratch)
    return bounded_pari_flx_rem(w, scratch, d, t, dt, p, out, scratch + 9)


@native
def bounded_pari_flxq_powu(
    w: UInt64Buffer,
    a: int,
    da: int,
    exponent: int,
    ...
    out: int,
    scratch: int,
    degree_work: IntegerBuffer,
    degree_work_start: int,
) -> int:
    ...
    if exponent == 2:
        return bounded_pari_flxq_sqr(w, a, da, t, dt, p, out, scratch)
    temporary = scratch + 18
    running = scratch + 81
    degree = da
    if exponent < 512:
        degree = bounded_pari_flx_copy(w, a, da, running)
        # Source highest bit is implicit, then square/multiply left to right.
        bit = exponent.bit_length() - 2
        while bit >= 0:
            degree = bounded_pari_flxq_sqr(
                w, running, degree, t, dt, p, temporary, scratch
            )
            bounded_pari_flx_copy(w, temporary, degree, running)
            if (exponent >> bit) & 1:
                degree = bounded_pari_flxq_mul(
                    w, running, degree, a, da, t, dt, p, temporary, scratch
                )
                bounded_pari_flx_copy(w, temporary, degree, running)
            bit -= 1
        return bounded_pari_flx_copy(w, running, degree, out)
    window = 2
    ...
    square = scratch + 27
    table = scratch + 36
    degrees = degree_work_start
    if degree_work_start < 0 or degree_work_start + 4 > len(degree_work):
        raise ValueError("short bounded Flxq degree workspace")
    square_degree = bounded_pari_flxq_sqr(w, a, da, t, dt, p, square, scratch)
    degree_work[degrees] = bounded_pari_flx_copy(w, a, da, table)
    for i in range(1, count):
        degree_work[degrees + i] = bounded_pari_flxq_mul(
            w,
            table + 9 * (i - 1),
...
def bounded_pari_flxq_powers(
    ...
    if length == 0:
        return 1
    degree_values[degree_slots + 1] = bounded_pari_flx_copy(w, a, da, out + 9)
    if length == 1:
        return 2
    degree_values[degree_slots + 2] = bounded_pari_flxq_sqr(
...
@native
def bounded_pari_flx_flxqv_eval(
    w: UInt64Buffer,
    q: int,
    dq: int,
    table: int,
    ...
    bounded_pari_flx_copy(w, last, degree, running)
