"""Resident rational-cache initialization, small-norm collection, logs and HNF.

Copyright (C) The PARI group. GPL-2.0-or-later, without warranty.
Experimental PARI 2.17.4 segment with prepared nf/factor-base/ideal packets.
...
from .row20_unreduced_small_norm import pari_collect_row20_unreduced_ideals
from .row20_relation_log_embeddings import pari_row20_append_relation_log_embeddings
from .hnfspec_complete import pari_hnfspec_complete


@native
def pari_connected_relation_hnf(
    ...
    chain_state holds phase, returned status, initialized relation count and
    collected relation count. Phase 3 is terminal and repeat calls are no-ops;
    phases 1/2 identify partial work after an exception and reject reuse.
    Collector 0 (exhausted schedule) and 1 (cache target) both proceed to HNF,
    as upstream small_norm returns void after either ordinary outcome.
    Negative collector status is terminal without HNF. An HNF frontier is
    terminal with its existing explicit partial-output contract. No failure
    is interpreted as a certified class group or regulator.

    All owners are distinct and preallocated. The initial target and factor
    base/permutation are prepared policy inputs. Caller supplies a fresh
    collector cursor and log prefix. HNF word conversion copies exact cached
    relation entries inside the core; this representation overhead is explicit.
    """
    if len(chain_state) < 4:
        raise ValueError("short connected relation HNF state")
    if chain_state[0] == 3:
        return int(chain_state[1])
    if chain_state[0] != 0:
        raise ValueError("cannot reuse a partial relation HNF stage")
    rows = int(len(relation))
    capacity = 10 * (rows + initial_additional) + 50
    if initial_additional < 0 or initial_target < 0 or initial_target > capacity:
        raise ValueError("invalid connected initial cache target")
    ...
    status = pari_collect_row20_unreduced_ideals(
        matrix,
        ideal,
        n,
        precision,
    ...
    chain_state[3] = columns
    if status != 0 and status != 1:
        chain_state[1] = status
        chain_state[0] = 3
        return status
    pari_row20_append_relation_log_embeddings(
        admission_matrix_m,
        admission_matrix_p,
        admission_matrix_e,
        generators,
