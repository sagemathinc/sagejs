...
const compilerRoot=path.resolve(process.argv[5]&&!process.argv[5].startsWith("--")?process.argv[5]:path.join(__dirname,"../.."));
...
(async()=>{
 ...
 const oneWord=process.argv.includes("--one-word");
 const timed=rows.map((row,i)=>oneWord?Math.min(Number(row[1]),Number(row[4]))===64:i<32);
...
static const int timed[228]={${timed.map(Number).join(",")}};
...
long count=timed[i]?1000:1;struct timespec a,b;sagejs_native_status error={0};
...
long count=timed[i]?1000:1;struct timespec a,b;clock_gettime(CLOCK_MONOTONIC,&a);
...
if(timed[i])total+=seconds(a,b);set_avma(outer);}
printf("%.17g\\n",total);pari_close();}
`;
 ...
 const report={qualified:false,checkedCases:228,timedCases:timed.filter(Boolean).length,oneWord,repetitions:1000,cpu,results,
