...
const readiness = require("./row8_phase6_prepared_timing_readiness.cjs");
...
const DEFAULT_PREPARED = "/scratch/sagejs-pari-fresh-prepared-corpus-v1/" +
  "prepared-row-08-993fd7dbd953844a1570baf326404cf3bf07223c721e3e7667d566764529fbd6.json";
...
async function main() {
  ...
  changed.precision = "193";
  assert.throws(() => authentication.authenticatePreparedNf(changed));
  const aggregate = JSON.parse(fs.readFileSync(path.resolve(
    positional[1] || DEFAULT_AGGREGATE)));
  ...
  assert(row);
  assert.equal(row.preparedAuthoritySha256, readiness.PREPARED_AUTHORITY_SHA256);
  ...
  process.stdout.write(`${JSON.stringify({
    schema: "sagejs.pari-class-group/row8-phase6-timing-readiness-check-v1",
    aggregateFreshCorrectnessAuthenticated: true,
    liveFreshCorrectnessExecuted: live !== null,
    live,
    mutationRejected: true,
    timingArmRejected: true,
    audit,
  }, null, 2)}\n`);
}
