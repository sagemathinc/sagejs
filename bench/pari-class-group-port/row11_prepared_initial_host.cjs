...
// Prepared-data-only host for the single row-11 initial root invocation.
...
const SOURCE = path.join(__dirname, "row11_prepared_initial_root.py");
const DEGREE = 4;
...
function parseSignature(source) {
  const match = source.match(/def pari_row11_prepared_initial_root\(([\s\S]*?)\n\) -> int:/);
...
function publishOwner(directory, owner) {
  ...
  const destination = path.join(directory,
    `row11-prepared-initial-${ownerSha256}.json.gz`);
...
async function computePreparedInitialRoot(payload, options = {}) {
  ...
  const fn = module.pari_row11_prepared_initial_root;
  ...
  const expectedInitialCount = 24n;
  ...
  const owner = {
    schema: "sagejs.pari-class-group/row11-prepared-initial-owner-v1",
