"""Prepared row-14 factor base and rational-relation root.
...
# ceiling fixed independently of the row-14 answer KC=799.  Logical views are
# derived only after KC has been computed inside the call.
ROW14_PREPARED_MAX_IDEALS = 1024
ROW14_PREPARED_MAX_ADDITIONAL = 16


@native
def pari_row14_prepared_initial_root(
    ...
    """Compute the row-14 prepared prefix through its 42 rational relations.
    ...
    if (
        ...
        raise ValueError("unsupported row14 prepared field corridor")
    ...
    if (
        ...
        raise ValueError("row14 root requires the neutral complete prime prefix")
    if len(base_state) < 7 or len(root_state) < 18:
        raise ValueError("short row14 prepared policy state")
    if (
        len(relation) < ROW14_PREPARED_MAX_IDEALS
        or len(relation_scratch) < ROW14_PREPARED_MAX_IDEALS
        or len(relation_basis) < ROW14_PREPARED_MAX_IDEALS * ROW14_PREPARED_MAX_IDEALS
    ):
        raise ValueError("short row14 neutral relation admission owners")
    maximum_records = (
        10 * (ROW14_PREPARED_MAX_IDEALS + ROW14_PREPARED_MAX_ADDITIONAL) + 50
    )
    if (
        len(relation_records) < maximum_records * ROW14_PREPARED_MAX_IDEALS
        ...
        raise ValueError("short row14 neutral relation record owners")
    ...
    if status != 0:
        raise ValueError("row14 degree catalog failed")
    ...
    if kcz != kcz2:
        raise ValueError("row14 unequal-bound honesty corridor is unsupported")
    if kc < 1 or kc > ROW14_PREPARED_MAX_IDEALS:
        raise ValueError("row14 factor base exceeds public admission ceiling")
    ...
    if written != kc:
        raise ValueError("row14 generated descriptor count mismatch")
    ...
    if (
        ...
        raise ValueError("short row14 active prime group owners")
    for i in range(kcz):
    ...
    if subcount < 1 or len(subfactor) < subcount or len(minidx) < kc:
        raise ValueError("short row14 subfactor/minidx owner")
    ...
    if additional > ROW14_PREPARED_MAX_ADDITIONAL:
        raise ValueError("row14 relation surplus exceeds public admission ceiling")
