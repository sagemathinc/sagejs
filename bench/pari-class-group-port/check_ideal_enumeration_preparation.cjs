...
`);
 ...
 const all=rows.map(r=>{
  ...
  const n=r.n;values.rounded_embedding=r.G0;values.rank=String(n);values.transform=Array(n*n).fill("0");values.flags=["0","0"];
...
`,path.resolve(__dirname,"../.."),path.resolve(__dirname,"../../src/lib")],{input:JSON.stringify([names,floatNames,all,rows,connected])});
 ...
 for(let index=0;index<rows.length;index++)for(const backend of ["javascript","gmp"]){
  ...
  if(connected){const n=r.n;assert.deepEqual(ints("transform"),Array.from({length:n*n},(_,p)=>BigInt(r.U[(p%n)*n+Math.floor(p/n)])),`${index} ${backend} U`);}
 }
 if(connected)for(const backend of ["javascript","gmp"]){
  ...
  values.rank--;
  if(backend==="gmp")for(const k of names)if(Array.isArray(values[k])&&!floatNames.includes(k))values[k]=f.packIntegerBuffer(values[k],64);
  assert.equal(f[backend](...names.map(k=>values[k])),1n,`${backend} rank dependency`);
 ...
 console.log(connected?"96 connected ranked LLL/ideal/embedding/QR/bound paths match PARI/CPython/JS/GMP; rank remains supplied, U is computed":"96 post-LLL ideal/embedding/QR/bound paths match PARI/CPython/JS/GMP exactly; U remains supplied scaffolding");
