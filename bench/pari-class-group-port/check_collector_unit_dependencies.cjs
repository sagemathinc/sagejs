...
for raw,e in zip(d['inputs'],d['expected']):
 ...
 primes=[p for p,count in enumerate(v['admission_prime_counts']) if count];initial=initialize(2,primes,[v['admission_prime_offsets'][p] for p in primes],[v['admission_prime_counts'][p] for p in primes],[1]*len(primes),v['ramification'],v['relation_state'],v['relation_basis'],v['relation_records'],v['relation_hashes'],v['relation_metadata'],v['relation'],v['relation_scratch'],v['n'],v['generators']);v['relation_state'][5]=400
 ...
 logs=[0]*(7*ru*last);completed=[0];append(v['admission_matrix_m'],v['admission_matrix_p'],v['admission_matrix_e'],v['generators'],v['relation_metadata'],last,n,v['admission_real_count'],128,completed,logs,[0]*n,[0]*(7*ru),[0]*3,[0]*3,[0]*64,[0]*64,[0]*64,[0]*64,[0]*128,initial);assert logs==list(map(int,e['logs'])),('logs',e['field']);assert completed==[last]
...
`,path.resolve(__dirname,'../..'),path.resolve(__dirname,'../../src/lib')],{input:JSON.stringify({inputs,expected,names:fixture.names,hsig})}));
 ...
 if(process.argv.includes('--native')){
  ...
  for(const backend of ['javascript','gmp'])for(let i=0;i<inputs.length;i++){
   ...
   const primes=v.admission_prime_counts.flatMap((x,p)=>x?[BigInt(p)]:[]);const initial=initialize[backend](2n,primes,primes.map(p=>v.admission_prime_offsets[Number(p)]),primes.map(p=>v.admission_prime_counts[Number(p)]),primes.map(()=>1n),v.ramification,v.relation_state,v.relation_basis,v.relation_records,v.relation_hashes,v.relation_metadata,v.relation,v.relation_scratch,v.n,v.generators);v.relation_state[5]=400n;
   const completed=[0n],logs=Array(Number(v.relation_state[1])*7*ru).fill(0n);const status=collect[backend](...fixture.names.map(([name])=>v[name]),128n,completed,logs,Array(n).fill(0n),Array(7*ru).fill(0n),Array(3).fill(0n),Array(3).fill(0n),Array(64).fill(0n),Array(64).fill(0n),Array(64).fill(0n),Array(64).fill(0n),Array(128).fill(0n),initial);assert.equal(status,0n);
