...
long precs[]={64,128,256,512};long sizes[]={0,1,63,64,65,127,128,129,191,192,193,513,1025};
for(long pi=0;pi<4;pi++)for(long ai=0;ai<13;ai++)for(long bi=1;bi<13;bi++)for(long sign=-1;sign<=1;sign+=2){
...
pari_printf("%Ps %Ps %ld %Ps %ld %ld\\n",a,b,p,out,signe(y)?bit_prec(y):0,expo(y));avma=av;
  ...
  const records=run.stdout.trim().split("\n").map(line=>line.split(" "));assert.equal(records.length,1248);
