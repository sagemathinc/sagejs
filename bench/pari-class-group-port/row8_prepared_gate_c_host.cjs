...
const ROWS = 143, DEGREE = 4, PLACES = 3, FIRST_COLUMNS = 150;
...
const APPEND_SHAPES = Object.freeze([
  Object.freeze({ oldState: Object.freeze([0, 7, 143, 0, 7, 14, 0, 150, 0]),
    newColumns: 1 }),
  Object.freeze({ oldState: Object.freeze([0, 8, 143, 0, 8, 0, 0, 151, 0]),
    newColumns: 1 }),
]);
...
function validateBoundary(prepared, root) {
  ...
  assert.equal(root.schema, "sagejs.pari-class-group/row8-prepared-initial-owner-v1");
  assert.equal(root.authority.preparedAuthoritySha256, prepared.authoritySha256);
  assert.deepEqual(root.rootState, ["1", "812", "812", "143", "93", "93",
    "143", "4", "143", "9", "150", "141", "4", "134", "143", "0",
    "1048576", "65537"]);
  assert.deepEqual(root.relations.state, ["9", "1550", "134", "7", "0", "150"]);
  assert.equal(root.relations.denseRecords.length, 9 * ROWS);
  ...
  assert.deepEqual(root.factor.subfactor, ["2", "4", "6", "8"]);
  ...
  assert.equal(root.capacity.logicalRecordCapacity, 1550);
}
...
function collectorInput(preparedEnvelope, root) {
  ...
  return {
    ...
    outer_mode: 1, outer_ru: PLACES, log_precision: 192, scalar_prefix_count: 9,
    ...
    relation_state: ["9", "1550", "134", "7", "0", String(FIRST_COLUMNS)],
    relation_basis: root.relations.basis,
    relation_records: [...root.relations.denseRecords,
      ...Array((1550 - 9) * ROWS).fill("0")],
    relation_hashes: [...root.relations.hashes, ...Array(1550 - 9).fill("0")],
    relation_metadata: [...root.relations.metadata, ...Array((1550 - 9) * 3).fill("0")],
    generators: [...root.relations.generators, ...Array((1550 - 9) * DEGREE).fill("0")],
...
function prepareHnfaddTransaction(storage, append, state, newColumns, explicit) {
  ...
  assert(shape, "unreviewed row-8 hnfadd transaction shape");
...
async function runPreparedGateC(prepared, root, options = {}) {
  ...
  const cv = first.cv;
  const initialState = Array.from(first.hnf.state).map(Number);
  const initialHRows = initialState[0], initialBColumns = initialState[2];
  const initialDepRows = ROWS - initialBColumns - initialHRows;
  const resident = {
    h: first.hnf.result_h.toArray().slice(0, initialHRows * initialHRows),
    dep: first.hnf.result_dep.toArray().slice(0, initialDepRows * initialHRows),
    b: first.hnf.result_b.toArray().slice(0, (ROWS - initialBColumns) * initialBColumns),
  ...
  const checkpoints = [snapshot(150)], expected = [151, 152], passTrace = [];
  // Publication of the first HNF advances the relation cache's HNF prefix.
  // The collector uses this cursor when choosing a continuation candidate;
  // leaving the pre-HNF zero here silently selects a different relation.
  writeIntegerSlot(cv.relation_state, 4, FIRST_COLUMNS);
  let checkpointIndex = 0, collectionPasses = 1, squash = 0;
  while (checkpointIndex < expected.length) {
    collectionPasses += 1;
    assert(collectionPasses <= 3, "row-8 continuation exceeded three authentic passes");
    // Row 8 reaches full ideal rank at the first HNF, but its analytic
    // acceptance rejects twice and requests one more relation each time.
    // This sentinel transaction has exactly those two reviewed retry slots;
    // the resulting terminal owner is independently replayed below.
    const need = Math.max(1, ROWS - resident.state[0] - resident.state[2]);
    ...
    const { search, outerPerm, outer, cache, schedule, completed, control, perm } =
      controlStorage;
    control.fill(0n);
    // Both reviewed row-8 retries follow regulator reconstruction action 5.
    // The resident driver records that A exists and that regulator data has
    // been attempted before preparing the next small-norm pass.  Omitting
    // these two latches changes the collector schedule and therefore does not
    // reproduce the genuine continuation, even when the requested need is 1.
    outer[14] = 1n;
    outer[15] = 1n;
    if (checkpointIndex === 0) outer[4] = 10n;
    ...
    // The source prepares F.L_jid at the dimension-ready frontier before the
    // regulator rejects.  Therefore the retry retains the complete HNF
    // permutation (need==0 search semantics), even though RELAT subsequently
    // requests one new independent relation.  Do not collapse it to a
    // one-ideal search here.
    ...
    nextControl[0] = ROWS;
    squash += 1;
    cv.search_count = BigInt(ROWS); cv.outer_mode = 1n;
    cv.outer_ru = BigInt(PLACES); cv.scalar_prefix_count = 9n;
