...
f = importlib.import_module(
    "bench.pari-class-group-port.bounded_prime_degree_catalog"
).bounded_pari_prime_degree_catalog
out = []
...
`,
  ...
  const kernel = require(built.modulePath).bounded_pari_prime_degree_catalog;
  const nativeOutputs = [];
  for (const backend of ["javascript", "gmp", "tagged"]) {
    for (let index = 0; index < rows.length; index += 1) {
      const args = packets[index].map((entry, argument) => {
        if (!Array.isArray(entry)) return BigInt(entry);
        if (argument === 6) return kernel.createUInt64Buffer(entry.map(BigInt));
      ...
      nativeOutputs.push({ backend, field: rows[index].field, state: got.args[20] });
    }
  }

  const core = fs.readFileSync(built.coreSourcePath, "utf8");
  const wordMul = core.match(
    /static int native_bounded_pari_flx_mul\([^;]*\)\n\{[\s\S]*?\n}\n/,
  );
  assert(wordMul, "missing bounded native Flx multiplication");
  ...
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "sagejs-bounded-prime-degree-catalog-"),
  );
  const result = {
    cases: rows.length,
    ...
    sourceHash: hash(
      fs.readFileSync(path.join(__dirname, "bounded_prime_degree_catalog.py")),
    ),
    coreHash: hash(fs.readFileSync(built.coreSourcePath)),
    coreSourcePath: built.coreSourcePath,
    boundedMulContainsMpz: /\bmpz_/.test(wordMul[0]),
    boundedMulMpzLocals: (wordMul[0].match(/^    mpz_t /gm) || []).length,
    boundedMulMpzOperations: (wordMul[0].match(/\bmpz_[a-z0-9_]+\(/g) || [])
      .length,
    boundedMulDirectWordAccesses: (wordMul[0].match(/\.data\[/g) || []).length,
    boundedMulExactBufferIndexes: (
