...
(async()=>{
 ...
 const source=fs.readFileSync(path.join(pari,'src/basemath/buch2.c'),'utf8');
 assert.equal(createHash('sha256').update(source).digest('hex'),'d8b09a54e51399c83f2faa92ccc3f1f70f41d660b1cb279738bc207ff553f87a');
...
for(long fi=0;fi<4;fi++){GEN nf=nfinit(gp_read_str(fields[fi]),192),M=nf_get_M(nf);long n=nf_get_degree(nf),r1=nf_get_r1(nf),ru=nbrows(M);
for(long k=1;k<=20;k++){pari_sp av=avma;long scalar=k>16;GEN v=cgetg(n+1,t_COL);for(long j=1;j<=n;j++)gel(v,j)=stoi(k<=4?(j==k?1:0):(j%2?k*j:-k-j));if(k==4&&n==3)gel(v,1)=gen_1;REL_t rel={scalar?stoi((k%2?-1:1)*(k-16)):v};
pari_printf("%ld %ld %ld ",n,r1,scalar);for(long j=1;j<=n;j++)pari_printf("%Ps ",scalar?rel.m:gel(v,j));
...
GEN out=get_log_embed(&rel,M,ru,r1,128);for(long i=1;i<=ru;i++){GEN z=gel(out,i);if(typ(z)==t_COMPLEX){pari_printf("2 ");emit(gel(z,1));emit(gel(z,2));}else{pari_printf("1 ");emit(z);emit(gen_0);}}pari_printf("\\n");avma=av;}}
pari_close();return 0;}`);
...
for ix,r in enumerate(json.load(sys.stdin)):
 ...
 got=f(*matrix,list(map(int,r['coords'])),r['n'],r['r1'],bool(r['scalar']),128,out,*work)
...
`,path.resolve(__dirname,'../..'),path.resolve(__dirname,'../../src/lib')],{input:JSON.stringify(cases)});
 ...
 for(const backend of ['javascript','gmp']){
  ...
  for(const [i,r]of cases.entries()){
   const out=make(r.expected.length),got=f[backend](...r.matrix.map(v=>v.map(BigInt)),r.coords.map(BigInt),BigInt(r.n),BigInt(r.r1),Boolean(r.scalar),128n,out,...work);
   assert.equal(got,BigInt((r.n+r.r1)/2));assert.deepEqual(Array.isArray(out)?out:out.toArray(),r.expected.map(BigInt),`${backend} ${i}`);
  }
 }
 console.log(JSON.stringify({cases:cases.length,backends:['PARI','CPython','javascript','gmp'],trace_sha256:createHash('sha256').update(trace).digest('hex'),core_bytes:fs.statSync(built.coreSourcePath).size}));
