...
const api = require("./row3_real_cubic_class_witness_coordinator.cjs");

const ROOT = path.resolve(__dirname, "../..");
const COORDINATOR = path.join(__dirname, "row3_real_cubic_class_witness_coordinator.cjs");

if (process.argv.length !== 4) {
  process.stderr.write("usage: check_row3_real_cubic_class_witness.cjs PRESENTATION_OWNER PRESENTATION_SHA256\n");
  process.exit(2);
}
const selected = path.resolve(process.argv[2]);
const digest = process.argv[3];
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "row3-class-witness-check-"));
...
try {
  function runCoordinator() {
    const run = spawnSync("timeout", ["600", "prlimit", "--as=4294967296",
      ...
      "--presentation-owner", selected, "--presentation-sha256", digest,
  ...
  const mutations = [
    value => { value.generator.presentationCoordinates[0] = "0"; },
    value => { value.generator.idealHnf[0] = String(BigInt(value.generator.idealHnf[0]) + 1n); },
    value => { value.quotient.generatorOrder = "3"; },
    value => { value.compactPrincipalWitness.relationExponents[0] = "0"; },
    value => { value.compactPrincipalWitness.principalGenerators[0] = "0"; },
    value => { value.orderRelation.factorBaseExponents[0] = "1"; },
    value => { value.exactIdealReplay.powerHnf[0] = "0"; },
  ...
  for (const mutate of mutations) {
    ...
    assert.throws(() => api.verifyOwner(changed, owner.ancestry), api.Row3ClassWitnessFailure);
  }
...
for label,mutate in [
 ...
 try:m.compose_row3_real_cubic_class_witness(changed,ancestry)
 except m.Row3ClassWitnessFailure:tests.append(label)
...
print(json.dumps(tests,separators=(',',':')))`;
  const semantic = spawnSync("timeout", ["600", "prlimit", "--as=4294967296",
    ...
    selected, digest], { cwd: ROOT, encoding: "utf8", timeout: 610_000,
  ...
  const rejected = spawnSync("timeout", ["600", "prlimit", "--as=4294967296",
    "--rss=4294967296", "--cpu=600", "--", "node", COORDINATOR,
    "--presentation-owner", changedPresentation, "--presentation-sha256", digest,
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    classGroup: ["6"],
    generatorOrder: "6",
    generatorIdealHnf: owner.generator.idealHnf,
    generatorPowerHnf: owner.exactIdealReplay.powerHnf,
