...
// Linux-only adapter for the pinned pristine PARI 2.17.4 row-19 prepared
...
const SOURCE = path.join(__dirname, "row19_phase6_pari_prepared_adapter.c");
...
function buildHelper() {
  ...
  assert.equal(process.platform, "linux", "row-19 PARI adapter is Linux-only");
  ...
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "sagejs-row19-pari-"));
  const executable = path.join(directory, "row19-pari-prepared-adapter");
...
function validateSample(sample) {
  exactKeys(sample, ["kernelNanoseconds", "processMaxRssKiB", "projection",
    "rng", "schema"], "PARI row-19 sample");
  assert.equal(sample.schema,
    "sagejs.pari-class-group/row19-pari-prepared-sample-v1");
...
class Client {
  constructor(build = buildHelper()) {
    ...
    this.exit = new Promise(resolve => this.child.once("exit", (code, signal) => {
      ...
      const error = new Error(
        `row-19 PARI helper exited (${code ?? signal}): ${this.stderr}`);
  ...
  nextLine() {
    ...
    assert.equal(this.closed, false, this.stderr || "row-19 helper is closed");
