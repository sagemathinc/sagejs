...
int main(void){pari_init(8000000,10000);long precs[]={64,128,192,256,512};
for(long pi=0;pi<5;pi++)for(long ti=0;ti<5;ti++)for(long pattern=0;pattern<5;pattern++)for(long sign=-1;sign<=1;sign+=2)for(long mode=0;mode<2;mode++){
...
for(long e=-1000;e<=100;e+=100)for(long ti=0;ti<5;ti++){long t=precs[ti];GEN x=real_0_bit(e),y=rtor(x,nbits2prec(t));printf("0 0 0 %ld %ld 0 0 %ld\\n",e,t,expo(y));}
pari_close();return 0;}`);
  ...
  const invalid=[
    ...
    [0,1n<<63n,64n,0n,2112n], [0,1n,64n,0n,64n],
