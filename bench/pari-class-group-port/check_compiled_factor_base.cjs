...
for(long bi=0;bi<8;bi++)for(long split=0;split<2;split++){
...
puts("");free_GRHcheck(&S);avma=av;
  ...
  const records=run.stdout.trim().split("\n").map(line=>{
    ...
    assert.equal(pos,r.length);return {n,c1,c2,primes,fullOffsets,fullCounts,degrees,logs,result,selectedPrimes,offsets,counts,complete,indices};
  });
...
`],{input:JSON.stringify(records),encoding:"utf8",timeout:30000});assert.equal(py.status,0,py.stderr);
  const built=await compileKernel({sourcePath:path.join(__dirname,"factor_base.py")}),mod=require(built.modulePath);
  for(const r of records)for(const backend of ["javascript","gmp"]){
    ...
    const result=mod.pari_prepared_factor_base[backend](...args);
    if(r===records[0]&&backend==="gmp")assert.throws(()=>mod.pari_prepared_factor_base.tagged(...args),/tagged native backend is not available for mixed exact\/Float64/);
  ...
  assert(records.some(r=>r.result[0]!==r.result[3]));
  assert(records.some(r=>r.complete.some((v,i)=>v==="0"&&r.counts[i]!=="0")));
  console.log(`${records.length} factor-base selections match FBgen/CPython/JS/GMP, including active/checking split and incomplete groups; tagged mixed capability explicitly unavailable`);
