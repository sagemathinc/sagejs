...
puts("");free_GRHcheck(&S);
  ...
  const records=run.stdout.trim().split("\n").map(line=>{
    ...
    const checks=r.slice(pos,pos+300),search=r.slice(pos+300);assert.equal(search.length,6);
    return {maximum,primes,offsets,counts,degrees,multiplicities,logs,checks,search};
  });
