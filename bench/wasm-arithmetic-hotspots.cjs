...
function discoverToolchain() {
  ...
  for (const generation of existsSync(store) ? readdirSync(store).sort().reverse() : []) {
    const cowasm = join(store, generation, "cowasm");
    const sdk = join(cowasm, "core/build/build/wasi-sdk/dist/wasi-sdk-next/native");
    const toolchain = {
    ...
    if (existsSync(toolchain.clang) &&
        existsSync(join(toolchain.gmpPrefix, "lib/libgmp.a"))) return toolchain;
  }
  throw new Error("a prepared WASI/GMP toolchain is required");
}
