...
def accumulate_relation_workspace(
    ...
    with NativeExactArena(arena_memory_limit, temporary_limit) as arena:
        ...
        initialized_first = native_exact_workspace_borrow_set(borrow, 0, first)
        initialized_second = native_exact_workspace_borrow_set(borrow, 1, second)
        index: uint64 = 0
        while index < rounds:
            updated_first = native_exact_workspace_borrow_addmul(
                borrow, 0, second, index
            )
            updated_second = native_exact_workspace_borrow_submul(
                borrow, 1, first, index
            )
            index += 1
        swapped = native_exact_workspace_borrow_swap(borrow, 0, 1)
...
def reset_relation_workspace(
    ...
    reset = native_exact_workspace_reset(
        workspace,
        expected_generation,
        specification_high,
        specification_low,
