...
function snapshot() {
  ...
  if (validation || access) visit("test/numerics/linear_algebra");
...
const access = process.argv[3] === "--access";
if (!output || ((lu || validation || access) ? process.argv.length !== 4 : process.argv.length !== 3)) throw new Error("usage: prepared-api-portable.cjs RECEIPT.json [--lu|--validation|--access]");
if (fs.existsSync(output)) throw new Error("receipt already exists");
const before = snapshot();
const tests = access ? ["test/numerics/linear_algebra/validation-access.cjs", "test/numerics/linear_algebra/validation-overflow.cjs"] : validation ? ["test/numerics/linear_algebra/validation-overflow.cjs"] : lu ? ["test/numerics/performance/packed-lu.cjs"] : ["test/numerics/performance/prepared-functions.cjs", "test/numerics/performance/prepared-root-api.cjs"];
...
const passed = result.status === 0 && same && new RegExp("(?:ℹ|#) pass " + (access ? "2" : (lu || validation) ? "1" : "3") + "\\b").test(result.stdout);
const python = process.env.PYTHON || (process.platform === "win32" ? "python" : "python3");
const version = spawnSync(python,["--version"],{encoding:"utf8",timeout:10000});
const receipt = {
  schema: access ? "sagejs.validation-access-qualification/v1" : validation ? "sagejs.validation-overflow-qualification/v1" : lu ? "sagejs.packed-lu-qualification/v1" : "sagejs.prepared-api-qualification/v1", passed,
  scope: (validation || access) ? "isolated-validation-regression-not-product-release" : lu ? "isolated-kernel-not-public-api" : "isolated-source-api-not-product-release",
  declared_source_commit: access ? "513996d0f" : validation ? "0f3f7472a" : lu ? "3311fd402" : "9c5066f72",
