...
async function main() {
  ...
  assert.equal(build.current, true, "fresh build required: " + build.reason);
  ...
  try {
    ...
    for (const [runtime, command, args] of [
      ...
      ["sagejs", process.execPath, ["--require", path.join(root, "test/helpers/assert-no-exact-numerical-load.cjs"),
    ...
    const report = {
      ...
      policy: { warmups: 3, samples: 7, observations: 20000,
        order: "CPython then Sage.js; none then summary; generic, prepared-dynamic, prepared-native (not paired)",
