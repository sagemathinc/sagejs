...
function certificationProgram(limit) {
  return [
    ...
    `    certified_pairs=rforest_genus3_local_factors(C,2,${limit},stage_observer=observe)`,
    "    certified=[dict(result,prime=prime) for prime,result in certified_pairs]",
    "except ImportError:",
    ...
    "cert_digest=0",
    "cert_counts={'unique':0,'fallback':0,'omitted':0,'other':0}",
    "candidate_total=0",
    "candidate_max=0",
    "primary_elements=0",
    "primary_scalars=0",
    ...
    "    status=completion.get('status',row.get('status','other'))",
    "    if status in cert_counts:",
    "        cert_counts[status]+=1",
    "    else:",
    "        cert_counts['other']+=1",
    "    diagnostics=completion.get('diagnostics',{})",
    "    enumeration=diagnostics.get('enumeration',{})",
    "    candidate_count=int(enumeration.get('candidate_count',0))",
    "    candidate_total+=candidate_count",
    "    candidate_max=max(candidate_max,candidate_count)",
    "    certificate=completion.get('certificate',{})",
    "    for kind in ('jacobian','twist'):",
    "        details=certificate.get(kind,{})",
    "        if kind=='jacobian':",
    "            primary_elements+=int(details.get('elements',0))",
    "            primary_scalars+=int(details.get('scalar_multiplications',0))",
    "        else:",
    "            twist_elements+=int(details.get('elements',0))",
    "            twist_scalars+=int(details.get('scalar_multiplications',0))",
    "        backend=str(details.get('backend','none'))",
    "        backend_counts[backend]=backend_counts.get(backend,0)+1",
    "    factor=completion.get('coefficients',completion.get('lpolynomial',row.get('lpolynomial')))",
    ...
    "{'rows':len(certified),'status_counts':cert_counts,'candidate_total':candidate_total,'candidate_max':candidate_max,'primary_elements':primary_elements,'primary_scalar_multiplications':primary_scalars,'twist_elements':twist_elements,'twist_scalar_multiplications':twist_scalars,'backend_counts':backend_counts,'event_counts':event_counts,'exact_stream_digest':cert_digest}",
  ].join("\n");
}
