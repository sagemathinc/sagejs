...
This benchmark decides between two production implementations that consume
the same canonical row-major `UInt64Buffer`:
...
storage. The legacy N-API timing is reported separately and starts with an
already constructed opaque FLINT matrix, so it is useful historical context
but is not the implementation-crossover baseline.
...
The production policy derived from this measurement is explicit:
...
If the declared FLINT artifact is unavailable, every operation uses its correct
same-source typed-Python fallback. These thresholds are tuning policy, not
semantics, and this benchmark is the executable mechanism for revisiting them.
