...
(async () => {
  ...
  try {
    ...
    const levels = process.argv.slice(2).filter(x => x !== "--decomposition").map(Number);
...
print(json.dumps({'system':'Sage.js','workload':'${decomposition ? "oldform-isogeny" : "T2-minus-1"}','level':${level},'dimension':J.dimension(),'rank':rank,'invariants':list(invariants),'construction_seconds':constructed-start,'smith_seconds':done-constructed,'warm_seconds':(warm-done)/10}))
`, { timeout: 600000 });
