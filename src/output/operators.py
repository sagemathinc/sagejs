...
def print_getitem(self, output):  # AST_Sub
    ...
    if (is_node_type(prop, AST_Number) or is_node_type(prop, AST_String)) or (
        is_node_type(prop, AST_SymbolRef) and prop.name and prop.name.startsWith("ρσ_")
    ):
        expr.print(output)
        output.print("["), prop.print(output), output.print("]")
        return
...
def print_delete(self, output):
    if is_node_type(self, AST_Seq) or is_node_type(self, AST_Array):
        values = self.to_array() if is_node_type(self, AST_Seq) else self.flatten()
...
def print_unary_prefix(self, output):
    ...
    if op is "!" and output.uses_python_truthiness():
        ...
        return
    if (op is "-" or op is "+" or op is "~") and not self.native_operator:
        output.print(
            "ρσ_operator_neg("
            if op is "-"
            else ("ρσ_operator_pos(" if op is "+" else "ρσ_operator_invert(")
        )
    else:
        output.print(op)
    if RegExp("^[a-z]", "i").test(op):
...
def write_smart_equality(self, output):
    def is_ok(x):
        return not (
            ...
            or is_node_type(x, AST_Set)
            or is_node_type(x, AST_Object)
            or is_node_type(x, AST_Statement)
            or is_node_type(x, AST_Binary)
            or is_node_type(x, AST_Conditional)
            or is_node_type(x, AST_BaseCall)
            or is_node_type(x, AST_SymbolRef)
        )
...
def is_native_typeof(node):
    return is_node_type(node, AST_Unary) and node.operator is "typeof"
...
def print_binary_op(self, output):
    if (
        ...
        self.left.print(output)
        output.print(", ")
        output.print_truth_test(AST_SymbolRef({"name": "ρσ_cond_temp"}))
        output.print(" ? ")
        if self.operator is "&&":
            self.right.print(output)
        else:
        ...
        def print_comparison(index):
            ...
            output.print("&&")
            output.space()
            output.print("(function(" + value_name(index + 2) + ") { return ")
        ...
        output.print(
            "(function(" + value_name(0) + ", " + value_name(1) + ") { return "
        )
        print_comparison(0)
        output.print("; })(")
        ...
        self.right.print(output)
        output.print(")")
    elif (self.operator is "==" or self.operator is "!=") and (
        is_native_typeof(self.left) or is_native_typeof(self.right)
    ):
        self.left.print(output)
        output.space()
        ...
        if nan_check is not None:
            ...
            # itself
            output.spaced(
                nan_check, "!==" if self.operator is "===" else "===", nan_check
            )
        else:
            output.spaced(self.left, self.operator, self.right)
    elif self.operator is "+":
        ...
        suffix = (
            "_exact("
            if (output.options.exact_integers and output.options.rational_division)
            else "("
        )
        ...
        (
            output.print("ρσ_operator_matmul("),
            self.left.print(output),
            output.comma(),
            self.right.print(output),
            output.print(")"),
        ...
        (
            output.print("ρσ_operator_floordiv("),
            self.left.print(output),
            output.comma(),
            self.right.print(output),
            output.print(")"),
...
def print_existential(self, output):
    key = (
        ...
        if self.after is None or jstype(self.after) is "string"
        else "e"
    )
...
def print_assignment(self, output):
    ...
    def print_unpack_source():
        ...
        elif is_node_type(self.right, AST_Array) and self.right.is_tuple:
    ...
    if self.operator is "=" and (
        is_node_type(left, AST_ItemAccess) or is_node_type(left, AST_Splice)
    ):
        print_target_assignment(left, output, lambda: self.right.print(output))
        return
    if (
        output.options.python_attributes
    ...
    if is_node_type(left, AST_Array):
        ...
        for index, element in enumerate(flat):
            if is_node_type(element, AST_Unary) and element.operator is "*":
        ...
        if star_index is not -1:
            ...
            for index, element in enumerate(flat):
                output.indent()
                target = element.expression if index is star_index else element
                ...
                def print_value():
                    if index is star_index:
                        ...
                        output.print(star_index)
                        output.comma()
                        output.print("ρσ_unpack.length - " + trailing_count)
                        output.print("))")
                    else:
...
def print_assign(self, output):
    ...
    if (
        ...
        output.print(")")
        return
    if is_node_type(self.left, AST_ItemAccess) and (
        self.operator in arithmetic_compound_functions
        or self.operator in compound_functions
    ):
        output.print("(")
        ...
        if is_compound_assign:
            ...
            def print_chain_body():
                for lhs in left_hand_sides:
                    ...
                    print_assignment(
                        AST_Assign(
                            {"left": lhs, "right": temp_rhs, "operator": self.operator}
                        ),
                        output,
                    )
                    output.end_statement()
...
def print_seq(output):
    ...
    if (
        ...
        or (is_node_type(p, AST_ItemAccess) and p.assignment is self)
        ...
        or (is_node_type(p, AST_ForIn) and p.object is self)
    ):
