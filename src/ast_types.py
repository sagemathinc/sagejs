...
class AST_Token(AST):
    ...
    properties = {
        ...
        "raw": "The exact source text of a numeric token",
        "is_integer": "Whether a numeric token is an integer literal",
        "numeric_suffix": "The Sage/Python suffix on a numeric token, such as r, L, or j",
        ...
        "delimiter_depth": "Number of open (), [], or {} delimiters before this token",
...
class AST_Scope(AST_Block):
    ...
    properties = {
        ...
        "annotated_locals": "[string*] annotated or deleted names requiring unbound checks",
...
class AST_Import(AST_Statement):
    ...
    properties = {
        ...
        "intrinsic": "[boolean] True for a compiler-only import which emits no module binding",
        "dynamic": "[boolean] resolve this module from sys.modules at runtime",
        "level": "[number] number of leading dots in a relative from-import",
        "star": "[boolean] import every public name from the module",
        "target_module": "[string] module namespace receiving a star import",
    }
...
class AST_Lambda(AST_Scope):
    ...
    properties = {
        ...
        "sequential_definition": "[bool*] assign this definition when its statement executes",
        "return_annotation": "[AST_Node?] The return type annotation provided (if any)",
        "return_annotation_text": "[string?] The source spelling of the return annotation",
...
class AST_Class(AST_Scope):
    ...
    properties = {
        ...
        "python_class": "[boolean] class calls use Python allocation and __new__ semantics",
        "lightweight": "[boolean] true if instances omit the generic Python identity slot",
        "sequence_class": "[boolean] expose class instances through the generic sequence proxy",
        "callable_instance_class": "[boolean] construct callable function objects as class instances",
        "bigint_fields": "[dict] instance fields known by the compiler to contain JS BigInts",
        "namedtuple_fields": "[string*] fields declared by a typing.NamedTuple class",
        "bound": "[string*] list of methods that need to be bound to self",
        "all_bound": "[string*] inherited and local eagerly bound method names",
        "shadowed_bound": "[string*] inherited bound methods shadowed by class values",
        "bind_inherited_methods": "[boolean] invoke a dynamically inherited eager-method binder",
        "decorators": "[AST_Decorator*] function decorators, if any",
        "module_id": "[string] The id of the module this class is defined in",
        "sequential_definition": "[boolean] assign this class when its statement executes",
...
class AST_VarDef(AST_Node):
    ...
    properties = {
        ...
        "is_global": "[boolean?] distinguish global from nonlocal declarations",
    }
...
class AST_ClassCall(AST_BaseCall):
    ...
    properties = {
        ...
        "classvar": "[boolean] call a callable class variable through the prototype",
    }
...
class AST_New(AST_Call):
    ...
    properties = {
        "python_class": "[boolean] this is a Python class call rather than explicit new",
    }
...
class AST_Seq(AST_Node):
    ...
    properties = {
        ...
        "parenthesized": "[bool] this sequence was explicitly nested in parentheses",
    }
    ...
    def to_array(self):
        ...
        while p:
            a.push(p.car)
            if p.cdr and (not is_node_type(p.cdr, AST_Seq) or p.cdr.parenthesized):
                a.push(p.cdr)
                break
            p = p.cdr
        return a
...
class AST_Sub(AST_PropAccess):
    ...
    properties = {
        "native_access": "[boolean] emit direct JavaScript property access without Python indexing",
    }
...
class AST_Unary(AST_Node):
    ...
    properties = {
        ...
        "native_operator": "[boolean] emit the JavaScript operator without Python dispatch",
        "inferred_type": "[string?] compiler-inferred primitive result type",
    }
...
class AST_Binary(AST_Node):
    ...
    properties = {
        ...
        "parenthesized": "[bool] Whether this binary expression was parenthesized",
        "native_operator": "[boolean] emit the JavaScript operator without Python dispatch",
        "inferred_type": "[string?] compiler-inferred primitive result type",
    }
    ...
    def _walk(self, visitor):
        return visitor._visit(
            self, lambda: [self.left._walk(visitor), self.right._walk(visitor)]
        )
    ...
    def traverse_chain(self):
        ...
        while True:
            if is_node_type(right, AST_Assign):
                ...
                continue
            if (
                is_node_type(right, AST_ItemAccess) or is_node_type(right, AST_Splice)
        ...
        while True:
            if is_node_type(next, AST_Assign):
                ...
                continue
            if (
                is_node_type(next, AST_ItemAccess) or is_node_type(next, AST_Splice)
...
class AST_Array(AST_Node):
    ...
    properties = {
        ...
        "is_native": "[boolean] whether this is a compiler-owned JavaScript array literal",
    }
...
class AST_SymbolDeclaration(AST_Symbol):
    ...
    properties = {"init": "[AST_Node*/S] array of initializers for this declaration."}
...
class AST_ImportedVar(AST_SymbolVar):
    ...
    properties = {"alias": "AST_SymbolAlias the alias for this imported symbol"}
...
class AST_SymbolFunarg(AST_SymbolVar):
    ...
    properties = {
        ...
        "annotation_text": "[string?] The source spelling of the annotation",
    }
...
class TreeWalker:
    ...
    def _visit(self, node, descend):
        ...
        ret = self.visit(node, (lambda: descend.call(node)) if descend else noop)
    ...
    def in_boolean_context(self):
        ...
        while i > 0:
            ...
            p = stack[i]
            if (
                is_node_type(p, AST_If)
                and p.condition is self
                or is_node_type(p, AST_Conditional)
                and p.condition is self
                or is_node_type(p, AST_DWLoop)
                and p.condition is self
                or is_node_type(p, AST_UnaryPrefix)
                and p.operator is "!"
