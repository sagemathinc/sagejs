...
class Rational(runtime.element):
    ...
    def __init__(
        ...
        if isinstance(numerator, Rational) and denominator is None:
            ...
            self._denominator = runtime.bigint_divexact(denominator, common)
    ...
    def _from_reduced(numerator: Any, denominator: Any) -> Rational:
        ...
        denominator = runtime.integer_bigint(denominator)
        if denominator <= 0:
            raise ValueError("a reduced rational must have positive denominator")
        answer = runtime.object.create(runtime.reflect.get(Rational, "prototype"))
    ...
    def _add_(self, other: Rational) -> Rational:
        ...
        common = runtime.bigint_gcd(left_denominator, right_denominator)
        left_quotient = runtime.bigint_divexact(left_denominator, common)
        right_quotient = runtime.bigint_divexact(right_denominator, common)
        return Rational(
            left_numerator * right_quotient + right_numerator * left_quotient,
    ...
    def _mul_(self, other: Rational) -> Rational:
        left_common = runtime.bigint_gcd(self._numerator, other._denominator)
        right_common = runtime.bigint_gcd(other._numerator, self._denominator)
        return Rational(
    ...
    def _truediv_(self, other: Rational) -> Rational:
        if other._numerator == runtime.bigint(0):
            raise ZeroDivisionError("rational division by zero")
        numerator_common = runtime.bigint_gcd(self._numerator, other._numerator)
        denominator_common = runtime.bigint_gcd(other._denominator, self._denominator)
        return Rational(
            runtime.bigint_divexact(self._numerator, numerator_common)
            * runtime.bigint_divexact(other._denominator, denominator_common),
            runtime.bigint_divexact(self._denominator, denominator_common)
            * runtime.bigint_divexact(other._numerator, numerator_common),
        )
    ...
    def _floor_quotient(self, other: Any) -> int:
        other_value = other if isinstance(other, Rational) else runtime.qq(other)
