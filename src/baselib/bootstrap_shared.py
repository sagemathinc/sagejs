...
def ρσ_bit(left, right, op, missing):
    return r"""%js (()=>{const a=typeof left,b=typeof right,e=(t,v)=>t==="boolean"||t==="bigint"||t==="number"&&Number.isSafeInteger(v);if(!e(a,left)||!e(b,right))return missing;if(a==="boolean"&&b==="boolean")return op===0?left&&right:op===1?left||right:left!==right;if(a==="number"&&b==="number"&&(left|0)===left&&(right|0)===right)return op===0?left&right:op===1?left|right:left^right;const v=op===0?BigInt(left)&BigInt(right):op===1?BigInt(left)|BigInt(right):BigInt(left)^BigInt(right),n=Number(v);return Number.isSafeInteger(n)?n===0?0:n:v})()"""
