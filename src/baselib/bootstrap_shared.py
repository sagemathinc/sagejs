...
def ρσ_attr(value, name, member):
    return r"""%js (()=>{const r=arguments.length<3,p=value==null?0:Object.getPrototypeOf(value),c=p&&_builtins_store_cache.get(p),h=Object.hasOwn;if(c&&c.get(name)===_builtins_descriptor_epoch.value&&!_builtins_instance_namespaces.has(value)&&(r?h(value,name):!h(value,"__setattr__"))){if(r)return value[name];let f=_builtins_instance_fields.get(value);if(f===undefined){f=new Set;_builtins_instance_fields.set(value,f)}const d=f.has(name)&&Object.getOwnPropertyDescriptor(value,name);if(d&&h(d,"value")){value[name]=member;return null}if(name!=="__proto__"&&Object.isExtensible(value)){value[name]=member;const n=Object.getOwnPropertyDescriptor(value,name);if(n&&h(n,"value")){f.add(name);return null}}Object.defineProperty(value,name,{value:member,writable:true,enumerable:true,configurable:true});f.add(name);return null}return r?ρσ_getattr_internal(value,name,ρσ_getattr_missing):ρσ_setattr(value,name,member)})()"""
