...
def _year_day(year, month, day):
    lengths = [31, 29 if _is_leap(year) else 28,
...
def _tuple(value, local):
    ...
    return (year, month, day, hour, minute, second, weekday,
            _year_day(year, month, day), -1 if local else 0)
...
def mktime(value):
    date = runtime.reflect.construct(_date_class, [
        value[0], value[1] - 1, value[2], value[3], value[4], value[5], 0,
    ])
...
def strftime(format, value=None):
    ...
    if value is None:
        value = localtime()
    replacements = {
        '%Y': str(value[0]).zfill(4), '%y': str(value[0] % 100).zfill(2),
        '%m': str(value[1]).zfill(2), '%d': str(value[2]).zfill(2),
        '%H': str(value[3]).zfill(2), '%M': str(value[4]).zfill(2),
        '%S': str(value[5]).zfill(2), '%a': _weekdays[value[6]],
        '%A': _weekdays[value[6]], '%b': _months[value[1] - 1],
        '%B': _months[value[1] - 1], '%j': str(value[7]).zfill(3),
