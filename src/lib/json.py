...
class JSONDecodeError(ValueError):
    def __init__(self, msg, doc, pos):
        ...
        ValueError.__init__(
            self,
            msg + ': line ' + str(self.lineno)
            + ' column ' + str(self.colno)
            + ' (char ' + str(pos) + ')',
        )
...
class JSONEncoder:
    ...
    def default(self, value):
        ...
        raise TypeError(
            'Object of type ' + type(value).__name__
...
def dump(obj, fp, **keywords):
    for chunk in (keywords.pop('cls', None) or JSONEncoder)(
        **keywords
    ).iterencode(obj):
        fp.write(chunk)
...
class JSONDecoder:
    ...
    def _string(self):
        ...
        while self.position < len(self.document):
            ...
            if self.strict and ord(character) < 32:
                self._error(
                    'Invalid control character at', self.position - 1)
            answer += character
        self._error('Unterminated string starting at', start)
    ...
    def _number(self):
        ...
        if (
            self.position < len(self.document)
            and self.document[self.position] == '.'
        ):
            floating = True
            self.position += 1
        ...
        if (
            self.position < len(self.document)
            and self.document[self.position] in 'eE'
        ):
            floating = True
            self.position += 1
            if (
    ...
    def _value(self):
        ...
        if self.document.startswith('Infinity', self.position):
            return self._constant(
                'Infinity', runtime.number.POSITIVE_INFINITY)
        if self.document.startswith('-Infinity', self.position):
            return self._constant(
                '-Infinity', runtime.number.NEGATIVE_INFINITY)
