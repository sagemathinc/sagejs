...
class HTTPConnection:
    ...
    def putheader(self, header, *values):
        ...
        self._headers.__setitem__(str(header), '\r\n\t'.join(str(value) for value in values))
    ...
    def _perform(self):
        ...
        if not target.startswith('http://') and not target.startswith('https://'):
            ...
            authority = self.host if is_default_port else self.host + ':' + str(self.port)
...
class HTTPSConnection(HTTPConnection):
    ...
    def __init__(self, host, port=None, *, timeout=None, source_address=None, context=None, blocksize=8192):
        del context
