...
def _ec_poly_mod(
    ...
    while len(answer) >= len(divisor) and not (len(answer) == 1 and answer[0] == 0):
        factor = answer[-1] * inverse % prime
        offset = len(answer) - len(divisor)
        for index, coefficient in enumerate(divisor):
            answer[offset + index] = (
                answer[offset + index] - factor * coefficient
            ) % prime
...
class EllipticCurveLocalData:
    ...
    def tamagawa_exponent(self) -> int:
        if (
            self._cp == 4
            and self._kodaira._roman == 1
            and self._kodaira._starred
            and self._kodaira._n % 2 == 0
        ):
            return 2
        return self._cp
...
def _ec_tate_large_prime(
    ...
    if j_denominator_valuation > 0:
        ...
        if difference == 6:
            residue = discriminant // _ec_bigint_power(
        ...
        raise ArithmeticError(
            "Tate's algorithm reached an impossible potentially multiplicative branch"
        )
...
def _ec_tate_small_prime(
    ...
    invariants = _ec_invariants(values)
    discriminant_valuation = _ec_valuation(invariants["discriminant"], prime)
    if discriminant_valuation == 0:
        return 0, 1, 1, None
    ...
    if invariants["b2"] % runtime.bigint(prime):
        split = ((-invariants["c6"]) % runtime.bigint(8 if prime == 2 else 3)) == 1
        tamagawa = (
            discriminant_valuation
            if split
            else (1 if discriminant_valuation % 2 else 2)
    ...
    if prime == 2:
        ...
        r_value = -runtime.number(invariants["b6"] % runtime.bigint(3))
    ...
    if invariants["b6"] % runtime.bigint(p3):
        ...
        tamagawa = 3 if invariants["b6"] % runtime.bigint(modulus) == p2 else 1
        return discriminant_valuation - 2, 4, tamagawa, 0

    if a6 % runtime.bigint(p3):
        t_value = 2 if prime == 2 else runtime.number(a3 % runtime.bigint(9))
        values = _ec_change_rst(values, 0, 0, t_value)
    a1, a2, a3, a4, a6 = values
    a21 = runtime.number((a2 % runtime.bigint(p2)) // runtime.bigint(prime))
    a42 = runtime.number((a4 % runtime.bigint(p3)) // runtime.bigint(p2))
    a63 = runtime.number((a6 % runtime.bigint(p4)) // runtime.bigint(p3))
    root_count, repeated_root = _ec_numroots3(a21, a42, a63, prime)
    ...
    if root_count == 2:
        if repeated_root:
            values = _ec_change_rst(values, repeated_root * prime, 0, 0)
        nu_value = 1
        pk = p2
        p2k = p4
        while True:
            ...
            alpha = 1
            roots, repeated_root = _ec_numroots2(alpha, beta, gamma, prime)
            if roots == 2:
                break
            if repeated_root:
                values = _ec_change_rst(values, 0, 0, repeated_root * pk)
            pk_previous = pk
            pk *= prime
            p2k *= prime
            nu_value += 1
            ...
            if roots == 2:
                break
            if repeated_root:
                values = _ec_change_rst(values, repeated_root * pk_previous, 0, 0)
            p2k *= prime
            nu_value += 1
        if prime == 2:
            tamagawa = 2 if gamma % 2 else 4
        else:
            tamagawa = 3 + _ec_legendre(beta * beta - alpha * gamma, 3)
    ...
    a32 = runtime.number((a3 % runtime.bigint(p3)) // runtime.bigint(p2))
    a64 = runtime.number((a6 % runtime.bigint(p5)) // runtime.bigint(p4))
    ...
    if repeated_root:
        values = _ec_change_rst(values, 0, 0, repeated_root * p2)
...
def _ec_lift_x(curve: Any, x_value: Any, all_roots: bool = False) -> Any:
    ...
    base = curve._base
    if getattr(x_parent, "_kind", None) in ["RDF", "RealField"] and getattr(
        base, "_kind", None
    ...
    if characteristic == 2 and (a1 != 0 or a3 != 0):
        raise NotImplementedError(
            "lift_x for long Weierstrass models in characteristic 2 is not implemented"
        )
...
class EllipticCurveIsogeny:
    ...
    def __init__(
        ...
        for point in self._kernel_representatives:
            ...
            if self._contains_x(kernel_x_values, x_value):
                continue
            gx_value = (base(3) * x_value + base(2) * a2) * x_value + a4 - a1 * y_value
            gy_value = 0 - base(2) * y_value - a1 * x_value - a3
            u_value = gy_value**2
        ...
        computed_codomain = runtime.reflect.apply(
            ...
            [
                ...
                [
                    ...
                    a3,
                    a4 - base(5) * v_value,
                    a6 - (a1**2 + base(4) * a2) * v_value - base(7) * w_value,
                ],
            ],
        )
        if codomain is not None:
            ...
            if list(codomain.ainvs()) != list(computed_codomain.ainvs()):
                raise NotImplementedError(
                    "post-composition with a codomain isomorphism is not implemented"
                )
    ...
    def _cyclic_points(
        ...
        while not point.is_zero():
            if len(answer) >= limit:
                raise ValueError("explicit Vélu kernel exceeds the safety limit")
            answer.append(point)
    ...
    def _generate_kernel(
        ...
        if isinstance(kernel, _untyped(_point_class)):
            ...
            for generator in generators:
                if not isinstance(generator, _untyped(_point_class)):
                    raise TypeError(
                        "polynomial and coefficient-list kernels are not implemented"
                    )
        if check:
        ...
        if len(generators) == 1:
            ...
            self._kernel_representatives = cyclic[1 : len(cyclic) // 2 + 1]
            return cyclic
        for generator in generators:
    ...
    def kernel_polynomial(self) -> Any:
        ring = _untyped(sage.PolynomialRing)(self._domain.base_ring(), "x")
        variable = ring.gen()
        answer = ring(1)
    ...
    def __call__(self, point: Any) -> Any:
        ...
        image_y = y_value
        for record in self._kernel_records:
            kernel_x, kernel_y, gx_value, gy_value, v_value, u_value = record
            difference = x_value - kernel_x
            inverse = difference ** (-1)
            inverse_squared = inverse**2
            inverse_cubed = inverse_squared * inverse
            image_x += v_value * inverse + u_value * inverse_squared
            y_term_zero = u_value * (2 * y_value + a1 * x_value + a3)
            y_term_one = v_value * (a1 * difference + y_value - kernel_y)
