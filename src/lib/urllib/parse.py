...
uses_netloc = ['', 'ftp', 'http', 'gopher', 'nntp', 'telnet', 'imap', 'wais', 'file', 'mms', 'https', 'shttp', 'snews', 'prospero', 'rtsp', 'rtsps', 'rtspu', 'rsync', 'svn', 'svn+ssh', 'sftp', 'nfs', 'git', 'git+ssh', 'ws', 'wss']
...
def _host_port(netloc):
    ...
    if port_text:
        try:
            ...
            raise ValueError("Port could not be cast to integer value as " + repr(port_text))
...
class SplitResult:
    ...
    def __iter__(self):
        return iter(runtime.math_tuple([
            self.scheme, self.netloc, self.path, self.query, self.fragment,
        ]))
    ...
    def __repr__(self):
        return (
            "SplitResult(scheme=" + repr(self.scheme)
...
def urlparse(url, scheme='', allow_fragments=True):
    ...
    return ParseResult(
        split.scheme, split.netloc, path, params,
...
def urljoin(base, url, allow_fragments=True):
    ...
    if target.netloc:
        return urlunparse((scheme, target.netloc, target.path, target.params, target.query, target.fragment))
...
def quote_from_bytes(value, safe='/'):
    ...
    for byte in data:
        character = chr(byte)
        answer += character if character in safe_bytes else '%' + hex(byte)[2:].upper().rjust(2, '0')
    return answer
