...
def _encode(value):
    ...
    if not module or not name or "<locals>" in name:
        raise PicklingError("can't pickle " + repr(cls) + ": no importable class name")
    ...
    if hasattr(value, "__getstate__"):
        ...
        raise PicklingError("can't pickle " + repr(cls) + ": no object state")
    return {
        _TAG: "object",
        "module": module,
        "name": name,
...
def _decode(value):
    ...
    if kind == "dict":
        return {_decode(pair[0]): _decode(pair[1]) for pair in value["items"]}
    if kind == "set":
