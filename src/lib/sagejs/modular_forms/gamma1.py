...
def _cache_for(attribute: str, space: Any) -> Any:
    # Certificates reference their parent. Keep this cycle parent-owned rather
    # than rooting every space and its potentially large matrices globally.
    cache = getattr(space, attribute, None)
    if cache is None:
        cache = runtime.map()
        setattr(space, attribute, cache)
    return cache
...
def character_components(
    ...
    cache = _cache_for("_gamma1_component_cache", _ambient(space))
...
class Gamma1DescentCertificate:
    ...
    def __init__(
        ...
        if self._raw_matrix.nrows() != self._expected_dimension:
            raise ArithmeticError("Gamma1 raw basis has the wrong component dimension")
...
def descent_certificate(
    ...
    if selected_kind == "New" and prime is None:
        prime = getattr(space, "_new_prime", None)
    cache = _cache_for("_gamma1_descent_cache", _ambient(space))
    key = selected_kind + ("" if prime is None else "|" + str(prime))
    cached = cache.get(key)
