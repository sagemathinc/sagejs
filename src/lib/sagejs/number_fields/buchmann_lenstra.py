...
def _dedekind_generator_lattice_is_order(
    ...
    """Check closure of `Z[a] + (generator(a)/modulus) Z[a]` once.

    The independently reconstructed HNF in the composite-Dedekind checker is
    exactly the lattice generated by `modulus*Z[a]` and
    `generator(a)*Z[a]`, divided by `modulus`.  It therefore contains the
    equation order and is stable under multiplication by `Z[a]`.  Such a
    lattice is a ring if and only if the square of its one extra module
    generator `generator(a)/modulus` belongs to the lattice.  Checking that
    single square replaces a degree-cubed multiplication table without
    weakening the closure proof.
    """
    degree = basis.degree
