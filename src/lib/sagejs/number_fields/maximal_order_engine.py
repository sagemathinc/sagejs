...
def inspect_maximal_order_selection(
    ...
    *,
    ...
    schedule = make_schedule(
        ...
        # asks for the hypothetical measured fallback schedule even when the
        # current evaluator has not installed the worker task graph.  Keep the
        # fail-closed live decision in `parallel_gate`, but do not let ambient
        # runtime state erase the caller's requested scheduler evidence.
        worker_capability=bool(worker_capability),
    )
