...
class RandomStream:
    ...
    A fixed seed, stream identifier, and method-call sequence produces the same
    integers and binary64 values in CPython and Sage.js on every target.
    """

    def __init__(self, seed: int, *, stream: int = 0) -> None:
        if isinstance(stream, bool) or not isinstance(stream, int) or stream < 0:
            raise ValueError("stream must be a nonnegative integer")
        self.seed = seed
        self.stream = stream
    ...
    def uniform(self, lower: float = 0.0, upper: float = 1.0) -> float:
        ...
        return lower_value + (upper_value - lower_value) * self.random()
    ...
    def normal(self, mean: float = 0.0, standard_deviation: float = 1.0) -> float:
        ...
        if self._normal_spare is not None:
            ...
            return mean_value + scale * standard
        while True:
            ...
            if 0.0 < radius < 1.0:
                ...
                return mean_value + scale * x * factor
    ...
    def spawn(self, index: int) -> "RandomStream":
        ...
        if isinstance(index, bool) or not isinstance(index, int) or index < 0:
            raise ValueError("child index must be a nonnegative integer")
    ...
    def state(self) -> dict[str, Any]:
        """Return a complete replay record for the next method call."""
        return {
    ...
    def from_state(cls, record: dict[str, Any]) -> "RandomStream":
        ...
        answer = cls(int(record["seed"]), stream=int(record["stream"]))
        state = int(record["state"])
        increment = int(record["increment"])
        count = int(record["draw_count"])
        if (
        ...
        if count < 0:
            raise ValueError("draw_count must be nonnegative")
    ...
    *,
    ...
    try:
        for index in range(size):
            values.append(_draw(distribution, stream, guard))
            guard.trace.append(
                "evaluation",
                evaluation=index + 1,
        ...
        if isinstance(distribution, (Binomial, Poisson)):
            support_ok = all(isinstance(value, int) and value >= 0 for value in values)
        ...
        guard.trace.append(
            "failure",
            data={"status": stopped.status, "completed_draws": len(values)},
            important=True,
            force=True,
        )
        return StatisticsResult(
