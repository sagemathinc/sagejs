...
def _check_execution(
    ...
    if elapsed_ms > problem.resource_budget.max_elapsed_ms:
        raise LinearAlgebraError(
            "elapsed_time_limit", "the elapsed-time resource budget was exhausted"
        )
...
def lu(
    ...
    *,
    ...
    factorization = lu_factorize(dense, pivot_threshold=pivot_threshold)
    record.append(
        "phase",
        data={
            "kind": "factorization",
    ...
    return _factorization_result(
        problem, plan, record, started, factorization, validate_lu(dense, factorization)
    )
...
def qr(
    ...
    *,
    ...
    record = _start_trace(problem, plan.method)
    started = time.monotonic()
    factorization = qr_factorize(dense, pivoted=pivoted, rank_tolerance=rank_tolerance)
    record.append(
        "phase",
        data={
            "kind": "factorization",
    ...
    return _factorization_result(
        problem, plan, record, started, factorization, validate_qr(dense, factorization)
    )
...
def cholesky(
    ...
    *,
    ...
    started = time.monotonic()
    try:
        factorization = cholesky_factorize(dense, symmetry_tolerance=symmetry_tolerance)
    ...
    return _factorization_result(
        ...
        started,
        factorization,
        validate_cholesky(dense, factorization),
    )
...
def _spectral_diagnostics(
    ...
    *,
    ...
    return singular_value_diagnostics(matrix, max_sweeps=max_sweeps, on_sweep=on_sweep)
...
def _select_solver(
    ...
    if method == "partial_pivot_lu":
        ...
        return factorization, factorization.solve
    if method == "column_pivoted_householder_qr":
        factorization = qr_factorize(matrix, pivoted=True)
        return factorization, factorization.solve_square
    if method == "cholesky":
        factorization = cholesky_factorize(matrix)
        return factorization, factorization.solve
...
def _refine_solution(
    ...
    *,
    ...
    _, initial_error = normwise_backward_error(
        matrix, solution, right, residual=residual
    )
    ...
    for iteration in range(1, max_refinement + 1):
        ...
        candidate_residual = independent_residual(matrix, candidate, right)
        _, candidate_error = normwise_backward_error(
            matrix, candidate, right, residual=candidate_residual
        )
...
def solve(
    ...
    *,
    ...
    dense = as_matrix(matrix)
    dense_right, was_vector = as_right_hand_side(right, dense.nrows)
    ...
    try:
        ...
        factorization, solve_correction = _select_solver(dense, selected)
        record.append(
...
def _overdetermined_solution(
    factorization: QRFactorization, right: DenseMatrix
) -> DenseMatrix:
    columns = factorization.original.ncols
    transformed = factorization.apply_q_transpose(right)
    ...
    for row in range(columns - 1, -1, -1):
        ...
        for column in range(solution_columns):
            ...
            value -= math.fsum(
                upper.entry(row, index) * permuted[index * solution_columns + column]
                for index in range(row + 1, columns)
            )
            permuted[row * solution_columns + column] = value / pivot
...
def _underdetermined_solution(
    ...
    for row in range(rows):
        ...
        for column in range(right_columns):
            ...
            y[row * right_columns + column] = value / pivot
    q = factorization.q()
    return q.multiply(DenseMatrix(rows, right_columns, y))
...
def least_squares(
    ...
    *,
    ...
    problem = _problem(
        ...
        dense,
        right=dense_right,
        method="column_pivoted_householder_qr",
        trace=trace,
        max_trace_events=max_trace_events,
        max_trace_bytes=max_trace_bytes,
    ...
    if dense.nrows >= dense.ncols:
        factorization = qr_factorize(dense, pivoted=True)
        solution = _overdetermined_solution(factorization, dense_right)
        solution_kind = "full_column_rank_least_squares"
    else:
        factorization = qr_factorize(dense.transpose(), pivoted=True)
        solution = _underdetermined_solution(dense, factorization, dense_right)
...
def determinant(
    ...
    *,
    ...
    validation = validate_lu(dense, factorization)
...
def inverse(
    ...
    *,
    ...
    inverse_matrix = factorization.solve(identity)
