...
def compress(data, compresslevel=9, *, mtime=0):
    ...
    return bytes(os._host_call(
        'compressData', 'gzip', list(bytes(data)), compresslevel))


def decompress(data):
    return bytes(os._host_call(
        'decompressData', 'gzip', list(bytes(data))))
...
class GzipFile:
    ...
    def read(self, size=-1):
        ...
        end = len(self._data) if size is None or size < 0 else min(
            len(self._data), self._position + size)
