...
| N6 | Full frozen four-platform/browser/npm/SEA qualification and reviewed defaults remain open. |

[PR #224](https://github.com/sagemathinc/sagejs/pull/224), source
`db7c806b0`, removes repeated retained-history serialization without changing
retention, byte limits or diagnostics. Fresh A/B/B/A runs against `5b307b65f`
retain identical observations locally and on quiet `bench-1`. In that host
campaign, a 128-event iteration-trace batch falls from 14.2–14.5 seconds to
295–306 ms; selected traced public roots, minima and ODEs also improve. These
are explicit workload observations, not fulfillment of the program's public
latency targets. Three production browser engines pass the trace oracle; the
unchanged payload budget passes. Evidence and limitations are in the PR's
`agents/numerical-trace-main-checkpoint.md` and retained result directory.
...
The next increments must be independently reviewable on current main, with
fresh identity-bound tests and evidence. Historical links below remain pinned
