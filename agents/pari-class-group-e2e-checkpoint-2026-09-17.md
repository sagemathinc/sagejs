...
| 4 — units/precision | Partial | The real H1 cubic performs exact source-derived units and retries from 192 to 2304 bits. The authentic mixed-quartic class logs pass packed `cleanarch` across CPython, JavaScript, GMP, tagged, and pristine PARI. Neutral 153088-bit packed Python/GMP probes match pristine PARI exactly for pi, log(2), exp(log(2)), and exp(log(2)+i). A source-matched full-product branch lowers ordinary Python `mx * my` to GMP `mpz_mul` above the pinned host's 3520-bit PARI threshold. The exact field-3 root/embedding rebuild matches all four realified roots and all 16 `make_M` entries bit-for-bit. Source-transparent real and complex logarithm paths now regenerate all 301 source-order principal-log columns: the immutable merged owner has 2,107 packed cells, matches all 32 selected-prefix columns byte-for-byte, and authenticates every batch and compiler-epoch transition. | Join the complete complex-log owner to the retained real/log matrix, then finish `cleanarch`, regulator, and unit reconstruction without answer-derived transformations. |
