...
| 4 — units/precision | Partial | The real H1 cubic performs exact source-derived units and retries from 192 to 2304 bits. The authentic mixed-quartic class logs pass packed `cleanarch` across CPython, JavaScript, GMP, tagged, and pristine PARI. Neutral 153088-bit packed Python/GMP probes match pristine PARI exactly for pi, log(2), exp(log(2)), and exp(log(2)+i). The first authentic field-3 scalar relation-log column is now regenerated from exact owners at 153088 bits and is packed-equal to PARI, with no 192-bit float reuse. | The mixed-quartic `getfu` path still returns PRECI at 192 bits. Exact prepared integral-basis provenance, high-precision quartic embeddings, the AGM real/complex logarithm branches for the remaining 300 columns, the regulator, and unit solve remain open. |
