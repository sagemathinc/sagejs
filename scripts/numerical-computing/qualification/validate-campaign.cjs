...
function validateTemplate(template, expectedRows) {
...
function main() {
  ...
  validateTemplate(readJson(path.join(matrixDirectory, "node-four-platform.template.json")), 4);
  validateTemplate(readJson(path.join(matrixDirectory, "full-runtime.template.json")), 12);
