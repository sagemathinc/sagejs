  ...
  for (const filename of fs.readdirSync(path.join(root, directory)).filter((name) => /\.ya?ml$/.test(name)).sort()) {
    ...
    for (const [id, job] of Object.entries(data.jobs)) {
      ...
      const steps = (job.steps || []).map((step, index) => {
        ...
        if (/\bgh\s+(?:api|workflow|run)\b|require-wasm-release\.cjs|artifact-set\.cjs\s+capture|artifact-handoff\.cjs\s+(?:verify|stage)/.test(step.run || "")) controlSteps.push({ key, index, name: step.name ?? null });
