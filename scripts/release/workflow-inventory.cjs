...
function workflowInventory(root, suppliedReview) {
  ...
  for (const filename of fs.readdirSync(path.join(root, directory)).filter((name) => /\.ya?ml$/.test(name)).sort()) {
    ...
    for (const [id, job] of Object.entries(data.jobs)) {
      ...
      const steps = (job.steps || []).map((step, index) => {
        ...
        if (/\bgh\s+(?:api|workflow|run)\b|require-wasm-release\.cjs|deployment-inputs\.cjs|artifact-set\.cjs\s+capture|artifact-handoff\.cjs\s+(?:verify|stage|prepare)|collect-macos-inspection\.cjs|publish-(?:github-assets|npm-packages)\.cjs|(?:publish-prepared|prepare-browser-deployment)\.cjs\s+(?!admit\b)|finalize-github-release\.cjs/.test(step.run || "")) controlSteps.push({ key, index, name: step.name ?? null });
  ...
  for (const entry of review.entries) {
    ...
    if (!Array.isArray(entry.helpers) || !Array.isArray(entry.requiresWorkflowSuccess) ||
        ...
        !Array.isArray(effects) || effects.some((item) => !item || !["dispatch", "rerun-job", "release-pointer", "release-assets", "npm-publication"].includes(item.kind) ||
          (item.kind === "release-pointer" ? item.target !== "github:releases/latest" :
            item.kind === "release-assets" ? item.target !== "github:release-assets" :
              item.kind === "npm-publication" ? item.target !== "npm:registry" : !jobKeys.has(item.target))) ||
