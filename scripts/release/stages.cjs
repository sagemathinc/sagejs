...
function plan(profile = "native", selected, target = targetForHost()) {
  ...
  const all = [
    ...
    { inputs: ["build/authenticated-numerical-product", "dist"], outputs: ["packages/flint-wasm/dist"] }),
    ...
    stage("portable", "correctness", [["node", "scripts/run-test-tier.cjs", "portable", "--resume"]]),
    stage("integration", "correctness", [["node", "scripts/run-test-tier.cjs", "integration", "--gate", "correctness", "--resume"]]),
