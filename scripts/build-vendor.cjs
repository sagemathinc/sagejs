...
const compilerOnly = arguments_.length === 1 && arguments_[0] === "--compiler";
if (arguments_.length !== 0 && !compilerOnly) {
  throw new Error(`unknown build-vendor option: ${arguments_.join(" ")}`);
}
...
console.log(
  compilerOnly
    ? "Bundled Python compiler parser backends"
    : "Bundled NumPy, symbolic, and foreign-language parser backends",
);
