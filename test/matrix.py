...
assert list(matrix_basis)[1] == matrix(QQ, [[0, 1, 0], [0, 0, 0]])
...
assert inverse == matrix(QQ, [[-2, 1], [QQ(3, 2), QQ(-1, 2)]])
assert ~A == inverse
assert A * inverse == identity_matrix(QQ, 2)
assert A.solve_right(vector(QQ, [1, 0])) == vector(QQ, [-2, QQ(3, 2)])
...
assert rectangular.row_space().basis_matrix() == matrix(ZZ, [[1, 0, -1], [0, 1, 2]])
...
assert matrix(QQ, [[QQ(1, 2), QQ(2, 3)]]).stack(matrix(ZZ, [[3, 4], [5, 6]])) == matrix(
...
assert rectangular.matrix_from_rows([1, 0]) == matrix(ZZ, [[1, 2, 3], [0, 1, 2]])
assert rectangular.matrix_from_columns([2, 0]) == matrix(ZZ, [[2, 0], [3, 1]])
assert rectangular.diagonal() == [0, 2]
assert A.trace() == 5

dependent = matrix(ZZ, 3, [1, 2, 3, 4, 5, 6, 7, 8, 9])
assert dependent.rref() == matrix(QQ, [[1, 0, -1], [0, 1, 2], [0, 0, 0]])
assert dependent.hermite_form() == matrix(ZZ, [[1, 2, 3], [0, 3, 6], [0, 0, 0]])
...
assert integer_kernel.basis_matrix() == matrix(ZZ, [[1, 1, -1], [0, 3, -2]])
...
left_kernel = matrix(ZZ, [[1, 2, 3], [2, 4, 6]]).left_kernel()
assert left_kernel.degree() == 2
assert left_kernel.basis_matrix() == matrix(ZZ, [[2, -1]])
assert vector(ZZ, [2, -1]) in left_kernel
assert matrix(ZZ, [[1, 2, 3], [2, 4, 6]]).kernel() == left_kernel

rational_kernel = matrix(QQ, [[1, 2, 3], [2, 4, 6]]).right_kernel()
...
assert sparse_rational_matrix * sparse_rational_kernel.T == zero_matrix(QQ, 20, 280)
...
assert line_sum.basis_matrix() == matrix(ZZ, [[1, 0], [0, 2]])
...
assert other_line_sum.basis_matrix() == matrix(ZZ, [[2, 0], [0, 1]])
...
assert (rational_plane_x + rational_plane_z).basis_matrix() == (identity_matrix(QQ, 3))
...
assert A.charpoly() == PolynomialRing(ZZ, "x")(
    PolynomialRing(ZZ, "x").gen() ** 2 - 5 * PolynomialRing(ZZ, "x").gen() - 2
)
...
path_order = 40
path_matrix = matrix(
    ZZ, path_order, path_order, lambda row, column: 1 if abs(row - column) == 1 else 0
)
...
for _ in range(2, path_order + 1):
    path_previous, path_expected = (
        path_expected,
        charpoly_x * path_expected - path_previous,
    )
...
noncyclic = matrix(ZZ, noncyclic_order, noncyclic_order, noncyclic_entry)
assert noncyclic.charpoly() == (
    (charpoly_x - 1) ** (noncyclic_order // 2)
    * (charpoly_x - 2) ** (noncyclic_order // 2)
)

huge = Integer("1606938044258990275541962092341162602522202993782792835301499")
large = matrix(ZZ, [[huge, 1], [1, huge]])
assert large.det() == huge * huge - 1

assert zero_matrix(QQ, 2, 3) == matrix(QQ, 2, 3)
assert diagonal_matrix([2, 3, 5]) == matrix(ZZ, [[2, 0, 0], [0, 3, 0], [0, 0, 5]])
...
uniform = matrix.random(ZZ, 4, 5, distribution="uniform", density=0.6)
...
assert finite.solve_right(vector(F5, [1, 0])) == vector(F5, [3, 4])
assert finite.solve_right(identity_matrix(F5, 2)) == finite_inverse
...
assert binary_fibonacci.charpoly()(binary_fibonacci) == (zero_matrix(F2, 2))

finite_singular = matrix(F5, [[1, 2, 3], [2, 4, 1]])
assert finite_singular.rank() == 1
assert finite_singular.right_nullity() == 2
assert not finite_singular.is_invertible()
assert finite_singular.rref() == matrix(F5, [[1, 2, 3], [0, 0, 0]])
finite_kernel = finite_singular.right_kernel()
assert finite_singular.right_kernel_matrix() == (finite_kernel.basis_matrix())
assert (
    finite_singular.right_kernel_matrix(basis="computed")
    == finite_kernel.basis_matrix()
)
assert finite_kernel.base_ring() is F5
assert finite_kernel.dimension() == 2
assert finite_singular * finite_kernel.basis_matrix().T == (zero_matrix(F5, 2, 2))
...
# generator.
binary_kernel_source = matrix(F2, [[1, 0, 0, 1], [0, 1, 0, 1]])
...
assert all(
    binary_kernel_source * element == vector(F2, [0, 0]) for element in binary_kernel
)
assert matrix(ZZ, [[2, 3, 5, 7]]) * vector(ZZ, list(binary_kernel[1])) == vector(
    ZZ, [12]
)
...
assert finite + matrix(ZZ, [[1, 1], [1, 1]]) == matrix(F5, [[2, 3], [4, 0]])
...
assert all(value == F5(value.lift()) for value in random_finite.list())
...
assert extension_matrix + extension_matrix == matrix(F9, [[2 * a, 2], [2, 0]])
assert extension_matrix - extension_matrix == zero_matrix(F9, 2)
assert -extension_matrix == matrix(F9, [[2 * a, 2], [2, 0]])
assert a * extension_matrix == matrix(F9, [[a * a, a], [a, 0]])
...
assert extension_matrix.solve_right(vector(F9, [a, 1])) == vector(F9, [1, 0])
assert str(extension_matrix.charpoly()) == "x^2 + (2*a)*x + 2"
assert extension_matrix.charpoly()(extension_matrix) == (zero_matrix(F9, 2))
assert extension_matrix.minpoly()(extension_matrix) == (zero_matrix(F9, 2))
...
assert extension_row * extension_kernel.basis_matrix().T == (zero_matrix(F9, 1, 2))
assert extension_row.left_kernel().dimension() == 0
assert matrix(F9, [[1], [a], [a + 1]]).kernel().dimension() == 2
assert matrix(ZZ, [[1, 2], [3, 4]]).change_ring(F9) == matrix(F9, [[1, 2], [0, 1]])
...
assert any(value not in [F9(0), F9(1), F9(2)] for value in random_extension.list())

F3t = PolynomialRing(GF(3), "t")
t3 = F3t.gen()
F9_explicit = GF(9, "u", modulus=t3**2 + 1)
u9 = F9_explicit.gen()
explicit_extension_matrix = matrix(F9_explicit, [[u9, 1], [1, 0]])
assert explicit_extension_matrix.base_ring() is F9_explicit
assert explicit_extension_matrix.det() == F9_explicit(2)
assert explicit_extension_matrix.inverse() == matrix(F9_explicit, [[0, 1], [1, 2 * u9]])
...
assert residue_matrix.inverse() == matrix(R36, [[14, 15], [15, 14]])
assert residue_matrix * residue_matrix.inverse() == (identity_matrix(R36, 2))
assert residue_matrix.solve_right(vector(R36, [1, 0])) == vector(R36, [14, 15])
assert str(residue_matrix.charpoly()) == "x^2 + 32*x + 31"
assert residue_matrix.charpoly()(residue_matrix) == (zero_matrix(R36, 2))
...
assert howell_source * residue_kernel.basis_matrix().T == (zero_matrix(R625, 3, 3))
