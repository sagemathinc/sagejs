    ...
    try {
      const result = await session.evaluate(
        [
          ...
          "    return saved_analysis(coefficients, scale, bound)",
          ...
          "summary = [len(analysis_calls), len(order_calls), fast.is_maximal(), fast.basis_matrix() == control.basis_matrix(), fast.discriminant() == control.discriminant(), fast_certificate == control_certificate, cached is fast, fast.maximal_order_trace() == {'schema': 'sagejs.number-fields/maximal-order-trace-v1', 'enabled': False, 'events': []}, control.maximal_order_trace()['enabled'], fast_certificate['component_certificate'] == control_certificate['component_certificate']]",
    ...
    try {
      const result = await session.evaluate(
        [
          ...
          "engine.field_analysis_resource.native_field_analysis = forbidden_analysis",
          ...
          "    engine.field_analysis_resource.native_field_analysis = deferred",
          ...
          "engine.field_analysis_resource.native_field_analysis = corrupt",
          ...
          "engine.field_analysis_resource.native_field_analysis = forged",
