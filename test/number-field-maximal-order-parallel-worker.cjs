    ...
    try {
      ...
      const result = await session.evaluate(
        [
          ...
          "import sagejs.number_fields.local_parallel_worker as worker",
          "R.<x> = QQ[]",
          "K.<a> = NumberField(x^2 - 88200)",
          "expected = K.maximal_order()",
          "native = engine.native_order_from_polynomial",
          "generated = engine._generated_round2_order_result",
          "decision = worker.public_worker_decision",
          ...
          "worker.public_worker_decision = enough_memory",
          "engine._ROUND2_MICROS_PER_UNIT = 2000000",
          "try:",
          "    G.<c> = NumberField(x^2 - 88200)",
          "    generated_order = G.maximal_order(trace=True)",
          "    generated_event = [event for event in generated_order.maximal_order_trace()['events'] if event['stage'] == 'native-local-orders'][-1]",
          "    engine._generated_round2_order_result = unavailable",
          ...
          "    engine._generated_round2_order_result = generated",
          "    worker.public_worker_decision = decision",
