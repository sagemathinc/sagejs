...
test("startup budgets can account for a measured platform-architecture cost", () => {
  assert.equal(startupDefaults(false, false, "linux", "arm64").budgetMs, 525);
  assert.equal(startupDefaults(false, false, "linux", "x64").budgetMs, 425);
  assert.equal(startupDefaults(true, false, "linux", "arm64").budgetMs, 435);
  assert.equal(startupDefaults(true, false, "darwin", "arm64").budgetMs, 300);
  assert.equal(startupDefaults(true, false, "linux", "x64").budgetMs, 370);
});
