...
`)
  ...
  // Hosted runners show enough scheduling and GC jitter to move the bulk
  // typed-array paths just above 5 ms and Apple Silicon deserialization across
  // a wide 10-24 ms distribution even after the runtime is warm. Retain
  // bounded sanity ceilings that still reject the old scalar fallback without
  // treating host noise as a serialization regression.
  const bulkBufferBudgetMs = 8;
  const deserializeBudgetMs = 30;
  assert.ok(pack < bulkBufferBudgetMs, `uint64 pack took ${pack.toFixed(2)} ms`);
  assert.ok(
    unpack < bulkBufferBudgetMs,
    `uint64 unpack took ${unpack.toFixed(2)} ms`,
  );
