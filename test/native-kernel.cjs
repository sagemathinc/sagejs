...
assert.equal(scalarExactIr.version, 26);
assert.equal(scalarFloatIr.version, 26);
assert.equal(reductionsIr.version, 26);
assert.deepEqual(
...
console.log(
  "Native Kernel v26 canonical isolated cores, records, reductions, buffers, provenance, P1, ABI, FFI, and fallback passed.",
);
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
