...
assert.equal(scalarExactIr.version, 40);
assert.equal(scalarFloatIr.version, 40);
assert.equal(reductionsIr.version, 40);
assert.deepEqual(
...
console.log(
  "Native Kernel v40 canonical isolated cores, records, reductions, buffers, provenance, P1, ABI, FFI, and fallback passed.",
);
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
