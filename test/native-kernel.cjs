...
assert.equal(scalarExactIr.version, 29);
assert.equal(scalarFloatIr.version, 29);
assert.equal(reductionsIr.version, 29);
assert.deepEqual(
...
console.log(
  "Native Kernel v29 canonical isolated cores, records, reductions, buffers, provenance, P1, ABI, FFI, and fallback passed.",
);
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
