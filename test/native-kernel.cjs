...
assert.equal(scalarExactIr.version, 32);
assert.equal(scalarFloatIr.version, 32);
assert.equal(reductionsIr.version, 32);
assert.deepEqual(
...
console.log(
  "Native Kernel v32 canonical isolated cores, records, reductions, buffers, provenance, P1, ABI, FFI, and fallback passed.",
);
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
