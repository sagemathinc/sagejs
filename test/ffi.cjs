...
test("FFI declarations are strict and generated modules are current", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "FmpqPolynomialPair", "FmpqMumfordResult", "FmpzModPolynomial",
  ...
  assert.match(runSage(["ffi", "check"]), /437 function\(s\)/);
...
test("native-boundary audit is a reviewed exact ratchet", () => {
  ...
  assert.equal(current.counts["declared-ffi"], 437);
  assert.equal(current.counts["declared-ffi-resource"], 32);
...
test("every N-API export has an exact symbol-level architecture decision", () => {
  ...
  assert.equal(inventory.exports.length, 320);
