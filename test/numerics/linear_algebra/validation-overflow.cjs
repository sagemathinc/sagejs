...
test("unrepresentable validation scales fail closed in CPython and Sage.js",()=>{
  const source=fs.readFileSync(path.join(__dirname,"validation-overflow.py"),"utf8");
  ...
  const directory=fs.mkdtempSync(path.join(os.tmpdir(),"sagejs-validation-overflow-"));
  try {
    ...
    assert.match(result.stdout,/validation overflow guards passed/);
