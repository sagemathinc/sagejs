...
test("ordinary Sage.js source preserves cancellation and final half-even rounding", {
  ...
  try {
    ...
    const fullCorpus = oracle().map((row) =>
      `([${row.values.map(pythonFloat).join(",")}], ${row.status}.0, ${pythonFloat(row.answer)})`
    ).join(",\n");
...
for values, status, expected in [${fullCorpus}]:
    output = [27.0, -31.0]
...
test("isolated packed sums preserve bounds under address and undefined-behavior sanitizers", {
  ...
  const temporary = mkdtempSync(path.join(tmpdir(), "sagejs-packed-sum-sanitizers-"));
  try {
    const ir = await lowerSource(readFileSync(sourcePath, "utf8"), sourcePath);
    const core = generateHostCore(ir, { moduleIdentity: "5ae0000000000002" });
    writeFileSync(path.join(temporary, "kernel_core.c"), core.source);
    writeFileSync(path.join(temporary, "kernel_core.h"), core.header);
    const rows = oracle().map((row) => `{
      const uint64_t bits[] = {${row.values.length
        ? row.values.map((hex) => `UINT64_C(0x${hex})`).join(",") : "0"}};
      check_case(bits, ${row.values.length}, ${row.status}, UINT64_C(0x${row.answer}));
    }`).join("\n");
    writeFileSync(path.join(temporary, "driver.c"), `
#include <assert.h>
#include <stdlib.h>
#include <string.h>
...
static void check_case(const uint64_t *bits, size_t count, double expected_status,
                       uint64_t expected_bits) {
  double *input = calloc(count + 1, sizeof(double));
  double *scratch = calloc(count + 1, sizeof(double));
  assert(input != NULL && scratch != NULL);
  for (size_t i = 0; i < count; i++) memcpy(input + i, bits + i, sizeof(double));
  input[count] = 17.0;
  scratch[count] = 19.0;
  double output[] = {27.0, -31.0};
  double returned = -1.0;
  sagejs_native_status status = {0, NULL};
  assert(sagejs_kernel_finite_sum(&status, &returned,
    (sagejs_float64_buffer){input, count},
    (sagejs_float64_buffer){scratch, count},
    (sagejs_float64_buffer){output, 1}, count));
  assert(status.code == SAGEJS_NATIVE_OK && returned == expected_status);
  assert(bits_of(output[0]) == expected_bits && output[1] == -31.0);
  assert(input[count] == 17.0 && scratch[count] == 19.0);
  for (size_t i = 0; i < count; i++) assert(bits_of(input[i]) == bits[i]);
  free(scratch);
  free(input);
}
int main(void) {
  ${rows}
  double input[] = {1.0}, scratch[] = {0.0}, output[] = {27.0, -31.0};
  for (int which = 0; which < 4; which++) {
    double returned = -1.0;
    sagejs_native_status status = {0, NULL};
    assert(sagejs_kernel_finite_sum(&status, &returned,
      (sagejs_float64_buffer){input, 1},
      (sagejs_float64_buffer){scratch, which == 1 ? 0 : 1},
      (sagejs_float64_buffer){output, which == 2 ? 0 : 1},
      which == 0 ? 2 : which == 3 ? UINT64_MAX : 1));
    assert(status.code == SAGEJS_NATIVE_OK && returned == 2.0);
    assert(output[0] == 27.0 && output[1] == -31.0);
  }
  return 0;
}
`);
    const executable = path.join(temporary, process.platform === "win32" ? "driver.exe" : "driver");
    const compiled = spawnSync(process.env.CC || "cc", [
      "-std=c11", "-O1", "-g", "-ffp-contract=off", "-fno-omit-frame-pointer",
      "-fsanitize=address,undefined", "-I", temporary,
      path.join(temporary, "kernel_core.c"), path.join(temporary, "driver.c"),
      "-lm", "-o", executable,
