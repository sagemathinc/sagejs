...
const builtinsNames = ["ρσ_synthetic_init_ends_at_object",
  "ρσ_skip_init_for_custom_new"];
...
test("shared custom-new guard preserves synthetic chains and epoch caching", () => {
  const objectInit = {};
  ...
  let lookups = 0;
  const api = context({
    _builtins_object_init: objectInit,
  ...
  assert.equal(api.ρσ_skip_init_for_custom_new(cls, synthetic), true);
  assert.equal(lookups, 1);
  assert.equal(api.ρσ_skip_init_for_custom_new(cls, synthetic), true);
  assert.equal(lookups, 1, "epoch-current answer is reused");
  epoch.value += 1;
  allocations.set(cls, objectNew);
  assert.equal(api.ρσ_skip_init_for_custom_new(cls, synthetic), false);
