...
test("recovery logarithms reuse the existing batch helper and preserve scalar semantics", () => {
  const transformed = recoveryLogSource(source);
  assert(transformed.length < source.length);
  assert.throws(() => recoveryLogSource(transformed));
  const result = spawnSync(pythonExecutable(), ["-c", String.raw`
import ast,collections,json,sys
data=json.load(sys.stdin)
ast.parse(data['transformed'])
...
`], { input: JSON.stringify({ source, transformed }), encoding: "utf8", timeout: 30000 });
