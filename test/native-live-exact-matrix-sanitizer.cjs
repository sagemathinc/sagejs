...
const sourcePath = resolve(root, "bench/native_live_exact_matrix.py");
...
int main(void)
{
    ...
    mpz_t seed, left, right, result, first, second, row, column;
    uint64_t row_count = 0;
    mpz_inits(seed, left, right, result, first, second, row, column, NULL);
    mpz_set_si(seed, -7);
    ...
    {
        assert(sagejs_kernel_live_matrix_addmul(
            &status, result, 2, 3, 4096, 1, 2, seed, left, right, 19));
        assert(status.code == SAGEJS_NATIVE_OK);
    }

    mpz_set_ui(result, 123);
    assert(!sagejs_kernel_live_matrix_addmul(
        &status, result, 2, 3, 191, 0, 0, left, left, right, 1));
    assert(status.code == SAGEJS_NATIVE_RANGE_ERROR);
    assert(mpz_cmp_ui(result, 123) == 0);

    assert(sagejs_kernel_live_matrix_operations(
        &status, first, second, &row_count, 4096, left, right));
    assert(status.code == SAGEJS_NATIVE_OK);
    assert(row_count == 2);

    mpz_set_si(row, -1);
    mpz_set_ui(column, 0);
    assert(!sagejs_kernel_live_matrix_index(
        &status, result, 256, row, column));
    assert(status.code == SAGEJS_NATIVE_RANGE_ERROR);
    ...
    assert(sagejs_kernel_live_matrix_addmul(
        &status, result, 2, 3, 4096, 1, 2, seed, left, right, 1));
    assert(status.code == SAGEJS_NATIVE_OK);

    mpz_clears(seed, left, right, result, first, second, row, column, NULL);
    return 0;
}
`;

test("live exact matrices clean up transactionally under platform sanitizers", {
  skip: process.platform === "win32" ? "sanitizer harness is Unix-only" : false,
}, async () => {
  const source = readFileSync(sourcePath, "utf8");
  const ir = await lowerSource(source, sourcePath);
  const core = generateHostCore(ir);
  const temporary = mkdtempSync(join(tmpdir(), "sagejs-live-matrix-asan-"));
  try {
    ...
    const executable = join(temporary, "live-matrix-sanitizer");
