...
test("native container methods expose explicit-self class descriptors", (context) => {
  const source = [
    ...
    "assert values == [1, 2, 9]",
    "values.append(11)",
    "saved_append = values.append",
    "saved_append(13)",
    "assert values == [1, 2, 9, 11, 13]",
    "assert list.append.__argnames__ == ['self', 'value']",
    "assert list.append.__module__ == 'sagejs._baselib.containers'",
    "assert list.append.__name__ == '_list_type_append'",
    "for args in ((values,), (values, 1, 2), ({}, 1)):",
    "    try:",
    "        list.append(*args)",
    "    except TypeError as error:",
    "        assert 'append' in str(error)",
    "    else:",
    "        assert False, 'invalid append call was accepted'",
    "class AppendOverride(list):",
    "    def append(self, value):",
    "        return list.append(self, value + 1)",
