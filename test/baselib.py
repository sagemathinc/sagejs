...
# Formal factorizations provide Sage-style sequence and arithmetic behavior.
F = Factorization([(5, 1), (2, 2), (2, 1)], unit=-1)
assrt.ok(isinstance(F, Factorization))
assrt.equal(repr(type(F)), "<class 'Factorization'>")
assrt.equal(repr(F), '-1 * 2^3 * 5')
assrt.equal(len(F), 2)
assrt.equal(repr(F[0]), '(2, 3)')
assrt.equal(repr(F[-1]), '(5, 1)')
assrt.equal(repr(list(F)), '[(2, 3), (5, 1)]')
assrt.equal(F.unit(), -1)
assrt.equal(F.value(), -40)
assrt.equal(repr(F.radical()), '2 * 5')
assrt.equal(F.radical_value(), 10)
assrt.ok(F.is_integral())
assrt.equal(repr(F * Factorization([(3, 2)])), '-1 * 2^3 * 3^2 * 5')
assrt.equal(repr(F**2), '2^6 * 5^2')
def mutate_factorization():
    F[0] = (3, 1)
assrt.throws(mutate_factorization, /does not support item assignment/)
