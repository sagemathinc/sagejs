...
test("absent policy preserves development auto while a stale release policy fails closed", (context) => {
  const item = enabledEmptyPolicy();
  context.after(() => fs.rmSync(item.root, { recursive: true, force: true }));
...
test("the disabled release policy retains the three historical Cantor envelopes", () => {
