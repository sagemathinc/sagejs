...
test("reinitializing bootstrap preserves active and suspended ownership", () => {
  const source = readFileSync(join(__dirname, '../src/baselib/bootstrap_shared.py'), 'utf8');
  const expression = source.match(/ρσ_handled_state = r"""%js ([\s\S]*?)"""/)[1];
  const context = createContext({RuntimeError: Error});
  const state = runInContext(expression, context);
  const error = new Error('owned');
  const token = state.enter(error);
  assert.equal(runInContext(expression, context), state);
  assert.equal(context.__sagejs_last_exception__, error);
  state.leave(token);
  assert.equal(context.__sagejs_last_exception__, null);
  context.state = state;
  const generator = runInContext(`state.wrap((function* () {
    const token = state.enter('suspended');
    try { yield 1; return globalThis.__sagejs_last_exception__; }
    finally { state.leave(token); }
  })())`, context);
  assert.equal(generator.next().value, 1);
  assert.equal(context.__sagejs_last_exception__, null);
  assert.equal(runInContext(expression, context), state);
  assert.equal(generator.next().value, 'suspended');
  assert.equal(context.__sagejs_last_exception__, null);
});
