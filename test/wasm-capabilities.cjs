...
test("all current Wasm-relevant capability kinds are reviewed", () => {
  ...
  assert.equal(counts["declared-ffi-function"].length, 439);
  assert.equal(counts["declared-ffi-resource"].length, 32);
  assert.equal(counts["production-kernel"].length, 47);
  assert.equal(counts["runtime-intrinsic"].length, 142);
  assert.equal(counts["specialist-capability"].length, 25);
  assert.equal(result.capabilities.length, 1005);
