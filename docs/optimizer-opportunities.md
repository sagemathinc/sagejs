...
Input identity: `4d858327598b2c7f44090633cf998354cfae13e9f27d3535a6c08b64f549ed68` (488 files, 10515354 bytes).
...
| Measure | Count |
| --- | ---: |
| Source modules compiled | 428 / 428 |
| Functions and methods compiled | 11988 |
| Loop-bearing functions and methods | 3793 |
| Loops in functions | 10712 |
| Selected optimized loops | 44 |
| Compiler-rejected loops | 2821 |
| Unrecognized loops | 7847 |
...
- Potential object-result sites: 71543
- Collection-allocation sites: 8581
- Known coercion sites: 15455
- Potential boundary-call sites: 72
- Unresolved call sites: 36087
...
| --- | ---: | --- |
| `dashboard.no-current-pass-claimed` | 7847 | No existing mathematical-domain pass proves this loop; profile it before adding a new domain. |
| `dashboard.dynamic-call-sites` | 7059 | Profile the calls, then inline, hoist, batch, or give the dominant call an authenticated coarse boundary. |
| `dashboard.no-mathematical-domain-evidence` | 6656 | Add precise annotations or an explicit domain contract only after profiling proves this loop matters. |
| `dashboard.comprehension-loop` | 3638 | Lower the comprehension through a dedicated packed/container representation before scalar optimization. |
| `dashboard.indexed-access-sites` | 2787 | Prove shape, element representation, aliasing, and ownership before selecting a packed lowering. |
| `bounded-integer.dynamic-call` | 2245 | Inline, hoist, or batch the dynamic call so the loop is one closed exact-integer operation graph. |
| `dashboard.control-flow-sites` | 1855 | Canonicalize the branches into a verified operation graph or add a domain-specific control-flow proof. |
| `bounded-integer.unsupported-iterator` | 1807 | Use a proved built-in `range` iteration shape or add a verifier for the required iterator semantics. |
...
| `dashboard.nested-loop-sites` | 1532 | Consider a fused multidimensional region with explicit shape and work bounds. |
