...
Input identity: `b087598c340024020f3df75193830c3c11bbebd723b38f735cc447b1adef6c86` (499 files, 10715722 bytes).
...
| Loops in functions | 11106 |
| Selected optimized loops | 46 |
| Compiler-rejected loops | 2884 |
| Unrecognized loops | 8176 |
...
- Potential object-result sites: 73498
- Collection-allocation sites: 8884
- Known coercion sites: 15852
- Potential boundary-call sites: 79
- Unresolved call sites: 37158
...
| --- | ---: | --- |
| `dashboard.no-current-pass-claimed` | 8176 | No existing mathematical-domain pass proves this loop; profile it before adding a new domain. |
| `dashboard.dynamic-call-sites` | 7351 | Profile the calls, then inline, hoist, batch, or give the dominant call an authenticated coarse boundary. |
...
| `bounded-integer.dynamic-call` | 2293 | Inline, hoist, or batch the dynamic call so the loop is one closed exact-integer operation graph. |
...
| `bounded-integer.mutable-buffer-access` | 1675 | Prove an owner-bound packed buffer, alias discipline, and transactional publication. |
| `dashboard.nested-loop-sites` | 1622 | Consider a fused multidimensional region with explicit shape and work bounds. |
| `bounded-integer.unsupported-operation:=` | 1442 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
...
| `bounded-integer.unsupported-operation:<` | 411 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
