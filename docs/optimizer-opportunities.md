...
Input identity: `02195ed0e25c945a790e19da1332854ae5f31c6f84f3c45956c182616e96cc6e` (514 files, 10906446 bytes).
Analyzed source bundle: `sha256:24dab8dad4b258275d20befbb906734659b2f957c47831632474951bfb712c29`; compiler identity: `sha256:1cd283ca1f356851e03a673ccf1ca158a825f571ff82fd830a30173cfc5cfbc8`.
...
| Functions and methods compiled | 11734 |
| Loop-bearing functions and methods | 3733 |
| Loops in functions | 10758 |
| Selected optimized loops | 49 |
| Compiler-rejected loops | 2970 |
| Unrecognized loops | 7739 |
...
- Potential object-result sites: 73449
- Collection-allocation sites: 8587
- Known coercion sites: 16302
- Potential boundary-call sites: 81
- Unresolved call sites: 36714
...
| --- | ---: | --- |
| `dashboard.no-current-pass-claimed` | 7739 | No existing mathematical-domain pass proves this loop; profile it before adding a new domain. |
| `dashboard.dynamic-call-sites` | 6950 | Profile the calls, then inline, hoist, batch, or give the dominant call an authenticated coarse boundary. |
| `dashboard.no-mathematical-domain-evidence` | 6426 | Add precise annotations or an explicit domain contract only after profiling proves this loop matters. |
| `dashboard.comprehension-loop` | 3646 | Lower the comprehension through a dedicated packed/container representation before scalar optimization. |
| `dashboard.indexed-access-sites` | 2695 | Prove shape, element representation, aliasing, and ownership before selecting a packed lowering. |
| `bounded-integer.dynamic-call` | 2399 | Inline, hoist, or batch the dynamic call so the loop is one closed exact-integer operation graph. |
| `bounded-integer.unsupported-iterator` | 1936 | Use a proved built-in `range` iteration shape or add a verifier for the required iterator semantics. |
| `dashboard.control-flow-sites` | 1816 | Canonicalize the branches into a verified operation graph or add a domain-specific control-flow proof. |
| `bounded-integer.mutable-buffer-access` | 1748 | Prove an owner-bound packed buffer, alias discipline, and transactional publication. |
| `bounded-integer.unsupported-operation:=` | 1524 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `dashboard.nested-loop-sites` | 1512 | Consider a fused multidimensional region with explicit shape and work bounds. |
| `bounded-integer.unsupported-control-flow` | 910 | Restructure the loop into supported transactional branches or add a verified control-flow lowering. |
| `bounded-integer.unsupported-operation:+=` | 844 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `bounded-integer.unsupported-operation:%` | 759 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `dashboard.unsupported-while-loop` | 724 | Prove a finite progress measure and transactional exits before lowering a `while` loop. |
| `bounded-integer.unsupported-operation:<` | 454 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `bounded-integer.unsupported-operation://` | 323 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `bounded-integer.unsupported-operation:>` | 281 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `bounded-integer.unsupported-operation:&&` | 248 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
...
| `bounded-integer.unsupported-operation:unary!` | 205 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
...
| `bounded-integer.unsupported-operation:<=` | 117 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
