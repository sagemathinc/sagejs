...
Input identity: `0157b1a31923795ae011477fa3598b5710c6ed847fb05c24ada9c43318a1d7ac` (498 files, 10679664 bytes).
...
| Functions and methods compiled | 12299 |
| Loop-bearing functions and methods | 3922 |
| Loops in functions | 11057 |
| Selected optimized loops | 46 |
| Compiler-rejected loops | 2875 |
| Unrecognized loops | 8136 |
...
- Potential object-result sites: 73230
- Collection-allocation sites: 8846
- Known coercion sites: 15790
- Potential boundary-call sites: 78
- Unresolved call sites: 37015
...
| --- | ---: | --- |
| `dashboard.no-current-pass-claimed` | 8136 | No existing mathematical-domain pass proves this loop; profile it before adding a new domain. |
| `dashboard.dynamic-call-sites` | 7314 | Profile the calls, then inline, hoist, batch, or give the dominant call an authenticated coarse boundary. |
| `dashboard.no-mathematical-domain-evidence` | 6855 | Add precise annotations or an explicit domain contract only after profiling proves this loop matters. |
| `dashboard.comprehension-loop` | 3798 | Lower the comprehension through a dedicated packed/container representation before scalar optimization. |
| `dashboard.indexed-access-sites` | 2853 | Prove shape, element representation, aliasing, and ownership before selecting a packed lowering. |
...
| `dashboard.control-flow-sites` | 1922 | Canonicalize the branches into a verified operation graph or add a domain-specific control-flow proof. |
| `bounded-integer.unsupported-iterator` | 1851 | Use a proved built-in `range` iteration shape or add a verifier for the required iterator semantics. |
| `bounded-integer.mutable-buffer-access` | 1669 | Prove an owner-bound packed buffer, alias discipline, and transactional publication. |
| `dashboard.nested-loop-sites` | 1612 | Consider a fused multidimensional region with explicit shape and work bounds. |
| `bounded-integer.unsupported-operation:=` | 1437 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `bounded-integer.unsupported-control-flow` | 794 | Restructure the loop into supported transactional branches or add a verified control-flow lowering. |
...
| `bounded-integer.unsupported-operation:%` | 763 | Replace or prove the one unsupported exact-integer operation, then pin the bounded-integer pass. |
| `dashboard.unsupported-while-loop` | 744 | Prove a finite progress measure and transactional exits before lowering a `while` loop. |
...
| `bounded-integer.unsupported-power` | 100 | Expand a small fixed power into ordered multiplications or add an exact bounded-power proof. |
