...
function generateC(ir) {
  ...
  const fieldFunctions = ir.functions.filter(
    (fn) => !["integer", "prime-field-matrix"].includes(fn.kernelKind),
  );
  ...
  const properties = ir.functions
    ...
    .join(",\n");
  if (
    primeFieldFunctions.length > 0 &&
    primeFieldFunctions.length === ir.functions.length
  ) {
    return `// Generated by Sage.js Native Kernel v9.
#include <limits.h>
#include <stdint.h>
#include <stdlib.h>
