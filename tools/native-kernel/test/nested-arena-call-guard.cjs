...
test("imported helper arena effects cannot bypass the lifetime guard", async () => {
  const ownedInner = inner.replace("matrix: FmpzMatrix, value: int", "value: int")
    .replace("fmpz_matrix_set_entry(matrix,", "fmpz_matrix_set_entry(scratch,");
  const imported = await lowerSource(preamble + ownedInner, "imported-arena.py");
  await assert.rejects(() => lowerSource(preamble +
    "from sagejs.test_arena import inner\n" +
    outer("inner").replace("inner(matrix, value)", "inner(value)"),
  "importer-arena.py", {
    functions: ["outer"],
...
test("a single arena cannot publish promotions into a caller-owned resource", async () => {
  await assert.rejects(() => lowerSource(preamble + inner, "borrowed-arena.py"),
    /NativeExactArena cannot retain external foreign resource matrix/);
  const local = inner.replace("matrix: FmpzMatrix, value: int", "value: int")
    .replace("    with NativeExactArena", "    matrix = fmpz_matrix(1, 1)\n    with NativeExactArena");
  await assert.rejects(() => lowerSource(preamble + local, "local-before-arena.py"),
    /NativeExactArena cannot retain external foreign resource/);
});
